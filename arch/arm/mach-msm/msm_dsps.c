...
#ifdef DSPS_DEBUG
#define DSPS_MSG(fmt, arg...) printk(fmt, ##arg)
#else  /* DSPS_DEBUG */
#define DSPS_MSG(fmt, arg...)
#endif /* DSPS_DEBUG */
/* iDPower ADD end */
			...
			unsigned int cmd, unsigned long arg)
{
	...
	u32 val = 0;
	/* iDPower ADD start */
	int buf[DSAFETY_ADC_MAX_CHANNELS];
	int charge_source;
	dsps_charge_current_t dsps_current;
	/* iDPower ADD end */
	...
	case DSPS_IOCTL_RESET:
		...
		break;
	/* iDPower ADD start */
	case DSPS_IOCTL_GET_CHARGER_STATE:
		ret = pm8921_is_battery_charging(&charge_source);
		DSPS_MSG("pm8921_is_battery_charging() ret = %d, source = %d\n", ret, charge_source);
		ret = put_user(ret, (u32 __user *) arg);
		break;
	case DSPS_IOCTL_GET_CURRENT:
		/* get current from SafetyIC */
		ret = safety_ic_current_temp_adc(DSAFETY_CURRENT_OUT, 0x03, buf);
		if(ret != 0)
		{
			DSPS_MSG("safety_ic_current_temp_adc() failed, ret = %d\n", ret);
			break;
		}
		DSPS_MSG("safety_ic_current_temp_adc(): VIBATDC = %d, Vmoni2 = %d\n",
					buf[0], buf[1]);
		dsps_current.safetyic_vibatdc = buf[0];
		dsps_current.safetyic_vmoni2 = buf[1];

		/* get current FuelGaugeIC */
		ret = pfgc_fgc_mstate(D_FGC_CURRENT_REQ, (void*)&dsps_current.fgic_current);
		if(ret != 0)
		{
			DSPS_MSG("pfgc_fgc_mstate() failed, ret = %d\n", ret);
			break;
		}
		DSPS_MSG("get_prop_batt_current(): fgic_current = %d\n",
					dsps_current.fgic_current);

		ret = copy_to_user((void __user *)arg, &dsps_current, sizeof(dsps_current));
		break;
	/* iDPower ADD end */
	default:
		ret = -EINVAL;

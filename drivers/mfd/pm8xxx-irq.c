...
/* for pmic resume irq print */
extern int msm_show_resume_irq_mask;
extern volatile atomic_t debug_suspend_status;
...
static int pm8xxx_irq_block_handler(struct pm_irq_chip *chip, int block)
{
	...
	for (i = 0; i < 8; i++) {
		if (bits & (1 << i)) {
			...
			/* for pmic resume irq (idpower debug) */
			if (msm_show_resume_irq_mask && atomic_read(&debug_suspend_status)){
				pr_info("pmic_IRQ (%d)\n", irq);
			}

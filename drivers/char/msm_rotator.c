...
{
	int bpp;
/* QUAD npdc300057381 START */
	int dscale_vfactor, dscale_hfactor;
/* QUAD npdc300057381 END */
...
/* QUAD npdc300057381 START */
	if (info->rotations & MDP_ROT_90) {
		dscale_vfactor = info->downscale_ratio & 0x03;
		dscale_hfactor = (info->downscale_ratio & 0x0C) >> 2;
	}else{
		dscale_hfactor = info->downscale_ratio & 0x03;
		dscale_vfactor = (info->downscale_ratio & 0x0C) >> 2;
	}
/* QUAD npdc300057381 END */
	...
	if (new_session) {
		...
		iowrite32((1  << 18) | 		/* chroma sampling 1=H2V1 */
			  ...
			  1 << 8 |			/* ROT_EN */
#if 0	/* QUAD npdc300057381 */
			  info->downscale_ratio << 2 |	/* downscale v ratio */
			  info->downscale_ratio,	/* downscale h ratio */
#else	/* QUAD npdc300057381 */
			  dscale_vfactor << 2 |		/* downscale v ratio */
			  dscale_hfactor,		/* downscale h ratio */
#endif	/* QUAD npdc300057381 */
...
/* QUAD npdc300057381 START */
	dscale_vfactor = info.downscale_ratio >> 2;
	dscale_hfactor = info.downscale_ratio & 0x03;
/* QUAD npdc300057381 END */
...
#else	/* QUAD npdc300057381 */
	    (dscale_vfactor > MAX_DOWNSCALE_RATIO) ||
	    (dscale_hfactor > MAX_DOWNSCALE_RATIO)) {
#endif  /* QUAD npdc300057381 */
...
#else	/* QUAD npdc300057381 */
		dst_w = info.src_rect.h >> dscale_hfactor;
		dst_h = info.src_rect.w >> dscale_vfactor;
#endif  /* QUAD npdc300057381 */
	} else {
#if 0   /* QUAD npdc300057381 */
...
#else	/* QUAD npdc300057381 */
		dst_w = info.src_rect.w >> dscale_hfactor;
		dst_h = info.src_rect.h >> dscale_vfactor;
#endif  /* QUAD npdc300057381 */
	}
...
#ifdef CONFIG_MSM_BUS_SCALING		/* BUS_BW_CUSTOM */
	msm_bus_scale_client_update_request(msm_rotator_dev->bus_client_handle, 0);
#endif	/* CONFIG_MSM_BUS_SCALING */	/* BUS_BW_CUSTOM */

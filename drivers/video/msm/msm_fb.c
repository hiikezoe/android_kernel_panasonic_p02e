...
int log_id = 4;
int log_disp = 0;
module_param_named(log_id, log_id, int, S_IRUGO | S_IWUSR | S_IWGRP);
module_param_named(log_disp, log_disp, int, S_IRUGO | S_IWUSR | S_IWGRP);
#endif
/* npdc300052072_e */
...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	if((pdata!=NULL) && (pdata->sysfs_register!=NULL))
	{
		err = pdata->sysfs_register(mfd->fbi->dev);
		if (err < 0)
			printk(KERN_ERR "Unable to create pfm interface.\n");
	}
#endif
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
	pdata = (struct msm_fb_panel_data *)mfd->pdev->dev.platform_data;
    if((pdata!=NULL) && (pdata->sysfs_unregister!=NULL))
    {
		pdata->sysfs_unregister(mfd->fbi->dev);
    }
#endif	
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
			if((pdata!=NULL) && (pdata->pre_on!=NULL))
			{
				pdata->pre_on(mfd);
			}
#endif
...
*/
	...
	default:
		if (mfd->panel_power_on) {
			...
			if((pdata!=NULL) && (pdata->pre_off!=NULL))
			{
				pdata->pre_off(mfd);
			}
#endif
...
static int msm_fb_open(struct fb_info *info, int user)
{
	...
	if (!mfd->ref_cnt) {
		...
		if ((result =
			...
			if ((mfd->index == 1) && (result == -EPERM) && (mfd->suspend.op_suspend == true)) {
				if (mfd->suspend.panel_power_on == false) {
					printk(KERN_ERR "msm_fb_open: make suspend.panel_power_on true\n");
			      ...
			      struct fb_info *info)
{
	...
	if (first_init){
		msleep(40); 
		ret = mipi_sharp_lcd_disp_on(mfd->pdev);
		if (ret < 0)
			printk("%s failed to send ON command \n",__func__);
		first_init = 0;
	}
#endif
...
/* npdc300052072_s */
#ifdef RESPONSE_DEBUG
	if((log_disp == 1) | (log_disp == 2) | (log_disp == 4)){
		printk("P_E\n");
	}
#endif
/* npdc300052072_e */
...
/* npdc300052072_s */
#ifdef RESPONSE_DEBUG
	if(((log_disp == 1) | (log_disp == 3)) && (req.id == log_id)){
		/* id=log_id‚Ìsession‚Ì‚ÝƒƒOo—Í */
		printk("O_E\n");
	}
	else if(log_disp == 2){
		/* id check */

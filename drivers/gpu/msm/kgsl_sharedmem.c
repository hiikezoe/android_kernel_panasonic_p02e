...
static void kgsl_sharedmem_free2(struct kgsl_memdesc *memdesc, unsigned int sglen_alloced);
			...
			size_t size, unsigned int protflags)
{
	...
	if (ret)
		kgsl_sharedmem_free2(memdesc, sglen);
...
static void kgsl_sharedmem_free2(struct kgsl_memdesc *memdesc, unsigned int sglen_alloced)
{
	...
	if (memdesc->sg)
		kgsl_sg_free(memdesc->sg, sglen_alloced);

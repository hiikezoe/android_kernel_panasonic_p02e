...
#define NCM_DEBUG(fmt, args...) printk(KERN_INFO "<NCM_debug>[%s:%s:%d]" fmt , __FILE__, __func__, __LINE__, ## args)
#else
#define NCM_DEBUG(fmt, args...)
...
#ifdef CONFIG_USB_ANDROID_NCM
	bool received_im;
	bool received_im_once;
	bool received_im_ignore;
#endif /* CONFIG_USB_ANDROID_NCM */
...
#ifdef CONFIG_USB_ANDROID_NCM
	char *received_im[2]   = { "USB_STATE=RECEIVED_IM", NULL };
#endif /* CONFIG_USB_ANDROID_NCM */
...
#ifdef CONFIG_USB_ANDROID_NCM
	if ((dev->received_im == 1) && (dev->received_im_once != 1)) {
		NCM_DEBUG("setting for uevent parameter(received_im message)\n");
		uevent_envp = received_im;
		dev->received_im_once = 1;
	} else
#endif /* CONFIG_USB_ANDROID_NCM */
	if (cdev->config) {
		uevent_envp = configured;
#ifdef CONFIG_USB_ANDROID_NCM_DEBUG
		NCM_DEBUG("uevent configured\n");
#endif /* CONFIG_USB_ANDROID_NCM_DEBUG */
		next_state = USB_CONFIGURED;
	} else if (dev->connected != dev->sw_connected) {
		uevent_envp = dev->connected ? connected : disconnected;
#ifdef CONFIG_USB_ANDROID_NCM_DEBUG
		NCM_DEBUG("change connect status\n");
#endif /* CONFIG_USB_ANDROID_NCM_DEBUG */
...
// <iDPower_002> add S
/* omf */
static int omf_function_init(struct android_usb_function *f, struct usb_composite_dev *cdev)
{
       return omf_init();
}

static void omf_function_cleanup(struct android_usb_function *f)
{
       return omf_cleanup();
}

static int omf_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
       return omf_bind_config(c);
}

static struct android_usb_function omf_function = {
       .name                   = "omf",
       .init                   = omf_function_init,
       .cleanup                = omf_function_cleanup,
       .bind_config    = omf_function_bind_config,
};
// <iDPower_002> add E
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (size >= sizeof(config->common->inquiry_string))
		return -EINVAL;
	strcpy( config->common->inquiry_string, buf );
	return size;
}
					...
					struct usb_configuration *c)
{
	...
	NCM_DEBUG("dev->received_im_ignore = 1\n");
	dev->received_im_ignore = 1;

	pr_info("%s MAC: %02X:%02X:%02X:%02X:%02X:%02X\n", __func__,
		ncm->ethaddr[0], ncm->ethaddr[1], ncm->ethaddr[2],
		ncm->ethaddr[3], ncm->ethaddr[4], ncm->ethaddr[5]);

	ret = gether_setup_name_ncm(c->cdev->gadget, ncm->ethaddr, "cdc_network");
	if (ret) {
		pr_err("%s: gether_setup failed\n", __func__);
		return ret;
	}

	return ncm_bind_config(c, ncm->ethaddr, ncm->vendorID,
				    ncm->manufacturer);
}
						...
						struct usb_configuration *c)
{
	...
	NCM_DEBUG("dev->received_im_ignore = 0\n");
	dev->received_im_ignore = 0;
	gether_cleanup_ncm();
}
			       ...
			       const char *buff, size_t size)
{
	...
	while (b) {
		...
		if (name) {
#ifdef CONFIG_USB_ANDROID_NCM
			NCM_DEBUG("android_usb: enable store name=%s", name);
#endif /* CONFIG_USB_ANDROID_NCM */
...
#ifdef CONFIG_USB_ANDROID_NCM_VERBOSE
		NCM_DEBUG("android_usb: idVendor        = 0x%04x\n", (int)(cdev->desc.idVendor));
		NCM_DEBUG("android_usb: idProduct       = 0x%04x\n", (int)(cdev->desc.idProduct));
		NCM_DEBUG("android_usb: bcdDevice       = 0x%04x\n", (int)(cdev->desc.bcdDevice));
		NCM_DEBUG("android_usb: bDeviceClass    = 0x%02x\n", (int)(cdev->desc.bDeviceClass));
		NCM_DEBUG("android_usb: bDeviceSubClass = 0x%02x\n", (int)(cdev->desc.bDeviceSubClass));
		NCM_DEBUG("android_usb: bDeviceProtocol = 0x%02x\n", (int)(cdev->desc.bDeviceProtocol));
		NCM_DEBUG("android_usb: call usb_gadget_connect\n");
#endif /* CONFIG_USB_ANDROID_NCM_DEBUG_VERBOSE */
		android_enable(dev);
		dev->enabled = true;
	} else if (!enabled && dev->enabled) {
#ifdef CONFIG_USB_ANDROID_NCM
		NCM_DEBUG("android_usb: call usb_gadget_disconnect");
#endif /* CONFIG_USB_ANDROID_NCM */
		android_disable(dev);
		list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
			if (f->disable)
...
#ifdef CONFIG_USB_ANDROID_NCM
		NCM_DEBUG("android_usb: already %s\n", dev->enabled ? "enabled" : "disabled");
#endif /* CONFIG_USB_ANDROID_NCM */
...
android_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *c)
{
	...
	spin_lock_irqsave(&cdev->lock, flags);
#ifdef CONFIG_USB_ANDROID_NCM
	if (c->bRequest == USB_REQ_NCM_IDENTIFICATION_MESSAGE) {
		if(c->bRequestType == USB_REQ_NCM_REQ_TYPE) {
			if((dev->received_im != 1) && (dev->received_im_once != 1)) {
#ifdef CONFIG_USB_ANDROID_NCM_DEBUG
				NCM_DEBUG("received_im req!! dev=%08x\n", (int)dev);
#endif /* CONFIG_USB_ANDROID_NCM_DEBUG */
				if(dev->received_im_ignore == 0) {
					dev->received_im = 1;
	 				pr_info("dev->received_im = %d", dev->received_im);
					schedule_work(&dev->work);
				}
				else {
					NCM_DEBUG("ignored received_im message.");
				}
			} else {
/*            NCM_DEBUG("dev->received_im_once is already 1.(%x)\n", dev->received_im_once); */
			}

		} else {
 			NCM_DEBUG("c->bRequestType != USB_REQ_NCM_REQ_TYPE(%x)\n", c->bRequestType);
		}

	} else 
#endif /* CONFIG_USB_ANDROID_NCM */
	if (!dev->connected) {
		NCM_DEBUG("new connect!! dev=%08x\n", (int)dev);
		dev->connected = 1;
		schedule_work(&dev->work);

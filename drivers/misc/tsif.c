...
#define D_FRC_BUF_SIZE            256*1024                                     /* 20120711 from jessie バッファ固定化対応 */
...
static void *g_data_buffer;                                                    /* 20120711 from jessie バッファ固定化対応 */
static dma_addr_t g_data_buffer_dma;                                           /* 20120711 from jessie バッファ固定化対応 */
...
/* 20120711			dev_info(&tsif_device->pdev->dev, */                       /* from jessie */
/* 20120711				 "Overflow detected\n"); */                            /* from jessie */
		}
...
/*		dma_free_coherent(NULL, TSIF_BUF_SIZE, */                              /* 20120711 from jessie バッファ固定化対応 */
/*				  tsif_device->data_buffer, */                                 /* 20120711 from jessie バッファ固定化対応 */
/*				  tsif_device->data_buffer_dma); */                            /* 20120711 from jessie バッファ固定化対応 */
...
/*	tsif_device->data_buffer = dma_alloc_coherent(NULL, TSIF_BUF_SIZE, */      /* 20120711 from jessie バッファ固定化対応 */
/*				&tsif_device->data_buffer_dma, GFP_KERNEL); */                 /* 20120711 from jessie バッファ固定化対応 */
	tsif_device->data_buffer = g_data_buffer;                                  /* 20120711 from jessie バッファ固定化対応 */
	tsif_device->data_buffer_dma = g_data_buffer_dma;                          /* 20120711 from jessie バッファ固定化対応 */
	if (!tsif_device->data_buffer)
		goto err;
...
/* 20120711		dev_info(&tsif_device->pdev->dev, "TSIF IRQ: OVERFLOW\n"); */  /* from jessie */
...
/* map I/O memory */
	...
	g_data_buffer = dma_alloc_coherent( NULL, D_FRC_BUF_SIZE, &g_data_buffer_dma, GFP_KERNEL ); /* 20120711 from jessie バッファ固定化対応 */
	if( !g_data_buffer )                                                       /* 20120711 from jessie バッファ固定化対応 */
	{                                                                          /* 20120711 from jessie バッファ固定化対応 */
		dev_err(&tsif_device->pdev->dev, "Failed to allocate DMA buffers\n");  /* 20120711 from jessie バッファ固定化対応 */
		goto err_attrs;                                                        /* 20120711 from jessie バッファ固定化対応 */
	}                                                                          /* 20120711 from jessie バッファ固定化対応 */
	dev_info(&tsif_device->pdev->dev, "data_buffer: %p phys 0x%08x\n",         /* 20120711 from jessie バッファ固定化対応 */
		g_data_buffer, g_data_buffer_dma);                                     /* 20120711 from jessie バッファ固定化対応 */
	return 0;
/* error path */
...
static int __devexit msm_tsif_remove(struct platform_device *pdev)
{
	...
	dma_free_coherent( NULL, D_FRC_BUF_SIZE, g_data_buffer, g_data_buffer_dma ); /* 20120711 from jessie バッファ固定化対応 */
	g_data_buffer = NULL;                                                      /* 20120711 from jessie バッファ固定化対応 */

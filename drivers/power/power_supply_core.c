...
static void power_supply_changed_work(struct work_struct *work)
{
	...
	if (psy->changed) {
		...
		wake_lock_timeout(&psy->uevent_wake_lock, 2 * HZ);	/* <npdc300034933> */
		spin_lock_irqsave(&psy->changed_lock, flags);
	}
	if (!psy->changed)
...
int power_supply_register(struct device *parent, struct power_supply *psy)
{
	...
	wake_lock_init(&psy->uevent_wake_lock, WAKE_LOCK_SUSPEND, "power-supply-uevent");	/* <npdc300034933> */
...
create_triggers_failed:
	...
	wake_lock_destroy(&psy->uevent_wake_lock);	/* <npdc300034933> */
	device_del(dev);

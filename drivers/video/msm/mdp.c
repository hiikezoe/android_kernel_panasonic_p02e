...
error_retry:
	...
	if (!(mdp_irq_mask & term)) {
		...
		int mdp_irq_mask_bk = mdp_irq_mask;
		mdp_irq_mask &= ~term;
		if (!mdp_irq_mask && mdp_irq_enabled) {
			mdp_irq_enabled = 0;
			if (disable_irq_limited(mdp_irq, 5000000) == -ETIME) {
				enable_irq(mdp_irq);
				mdp_irq_mask = mdp_irq_mask_bk;
				mdp_irq_enabled = 1;
				spin_unlock_irqrestore(&mdp_lock, irq_flags);
				printk(KERN_ERR "%s: MDP IRQ disable_irq_limited error\n", __func__);
				goto error_retry;
			}
		}
	}
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
	mdp4_overlay_mdp_perf_current_set(index);
...
static int mdp_probe(struct platform_device *pdev)
{
	...
	if (!(mfd->cont_splash_done))
		...
		mdp_bus_scale_update_request(OVERLAY_BUS_SCALE_TABLE_MAX);
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
unsigned int mdp_get_bus_scale_level(unsigned int bus_bw)
{
	unsigned char count = 0;

	for(count = 0; count < mdp_pdata->mdp_bus_scale_table->num_usecases; count++){
		if( bus_bw < mdp_pdata->mdp_bus_scale_table->usecase[count].vectors[0].ab )
		{
			if( 2 >= count ){
				return OVERLAY_PERF_LEVEL_MIN;
			}
			else{
				return mdp_pdata->mdp_bus_scale_table->num_usecases - count;
			}
		}
	}

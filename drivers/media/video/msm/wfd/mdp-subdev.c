...
extern void mdp4_writeback_poverlay(struct fb_info *);
...
void mdp_force_pause(void *arg) {
    struct mdp_instance *inst = (struct mdp_instance *)arg;
    if (inst->mdp == NULL ) {
        printk("mdp_force_pause: arg is null\n");
	return;
    }
    mdp4_writeback_poverlay(inst->mdp);
}

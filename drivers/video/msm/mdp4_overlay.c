...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
	.mdp_bw = OVERLAY_PERF_LEVEL_MIN,
#else	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* BUS_BW_CUSTOM */
	.mdp_bw = OVERLAY_PERF_LEVEL4,
#endif	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* BUS_BW_CUSTOM */
};
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
unsigned char layer_change = 1;			/* Initialization by the bus bw change permission */
#endif	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* BUS_BW_CUSTOM */
...
#ifdef	CONFIG_FB_MSM_MIPI_DSI_TMD		/* npdc300056605 */
			else if ( pipe->alpha_enable &&
					!(pipe->op_mode & MDP4_OP_SRC_DATA_YCBCR) &&
					pipe->dst_h > pipe->src_h &&
					(( pipe->dst_h * 10000 ) / pipe->src_h) < 10050)
				pipe->op_mode |= MDP4_OP_SCALEY_PIXEL_RPT;
#endif	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* npdc300056605 */
		...
		if (pipe->pipe_type == OVERLAY_TYPE_VIDEO) {
			if (pipe->flags & MDP_BACKEND_COMPOSITION &&
				...
				pipe->op_mode |= MDP4_OP_SCALEX_PIXEL_RPT;
					...
					((pipe->dst_w * 10000) / pipe->src_w) < 10050 )
				pipe->op_mode |= MDP4_OP_SCALEX_PIXEL_RPT;
#endif  /* CONFIG_FB_MSM_MIPI_DSI_TMD */        /* npdc300056605 */
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
void mdp4_overlay_rgb_blackscreen_setup(struct mdp4_overlay_pipe *pipe)
{
	char *rgb_base;
	uint32 src_size, src_xy, dst_size, dst_xy;
	uint32 format, pattern;
	...
	outpdw (MDP_BASE + 0x41008, 0x00000000);  /* set MDP_RGB1_CONSTANT_COLOR to black */
	format = format | MDP4_FORMAT_SOLID_FILL ;
	outpdw(rgb_base + 0x0050, format);	/* MDP_RGB_SRC_FORMAT */
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
void mdp4_mixer_stage_gfx_commit(int mixer)
{
	u32 pull_mode = 0;
	/* only MIXER0 */
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
...
void mdp4_mixer_stage_gfx_up(struct mdp4_overlay_pipe *pipe)
{
...
#endif /*CONFIG_FB_MSM_MIPI_DSI_TMD*/
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
		/* allow bus bw down from "mdp4_dsi_video_overlay" */
		if(!(pipe->solid_fill)){
			layer_change = 1;
			pipe->solid_fill = 1;
		}
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
		/* bus bw rise for a SOLID_FILL layer */
		if(pipe->solid_fill){
			layer_change = 1;
			pipe->solid_fill = 0;
			mfd = mdp4_overlay_dsi_video_get_mfd();
			if( mfd != NULL ) {
				  ...
				  struct mdp4_overlay_pipe *pipe)
{
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
		pipe->req_bw = OVERLAY_PERF_LEVEL_MIN;
#else	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* BUS_BW_CUSTOM */
	 ...
	 */
#ifndef CONFIG_FB_MSM_MIPI_DSI_TMD		/* npdc300057307 */
...
#endif  /* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* npdc300057307 */
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* npdc300057307 */
	if ((pipe->flags & MDP_DEINTERLACE) && (pipe->req_clk < mdp_max_clk)) {
		pr_info("%s deinterlace requires max mdp clk.\n",
			__func__);
		pipe->req_clk = mdp_max_clk;
        }
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
/* OVERLAY_BUS_SCALE_TABLE_BASE is moved to mdp4.h */
#else	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* BUS_BW_CUSTOM */
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
	bus_bw = mdp4_calc_pipe_mdp_bw_single( pipe );
	pipe->req_bw = mdp_get_bus_scale_level( bus_bw );
#else	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* BUS_BW_CUSTOM */
	if (pipe->pipe_type == OVERLAY_TYPE_BF) {
		pipe->req_bw = OVERLAY_PERF_LEVEL4;
		return 0;
			      ...
			      struct mdp4_overlay_pipe *plist)
{
	u32 worst_mdp_clk = 0;
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
	u32 worst_mdp_bw = OVERLAY_PERF_LEVEL_MIN;
#else	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* BUS_BW_CUSTOM */
	...
	int ret = -EINVAL;
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
	unsigned int total_bus_bw = 0;
	unsigned int single_bus_bw = 0;
	unsigned char mix_num = 0;
	unsigned char mix_stage = 0;
	unsigned char solid_fill = 0;

	unsigned char j, k, l;
	unsigned int base_x_start = 0;
	unsigned int base_x_end = 0;
	unsigned int base_y_start = 0;
	unsigned int base_y_end = 0;
	unsigned int over_x_start = 0;
	unsigned int over_x_end = 0;
	unsigned int over_y_start = 0;
	unsigned int over_y_end = 0;
	unsigned char solid_fill_array[OVERLAY_PIPE_MAX];
	unsigned char overlap[OVERLAY_PIPE_MAX][OVERLAY_PIPE_MAX];
	struct mdp4_overlay_pipe *pipe2 = plist;
#endif	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* BUS_BW_CUSTOM */
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
	memset(&solid_fill_array[0], 0, sizeof(solid_fill_array));
	memset(&overlap[0][0], 0, sizeof(overlap));
#endif	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* BUS_BW_CUSTOM */
	...
	for (i = 0; i < OVERLAY_PIPE_MAX; i++, pipe++) {
		...
		if (pipe->req_clk > mdp_max_clk) {
			if (pipe->mixer_num == MDP4_MIXER0)
#ifdef	CONFIG_FB_MSM_MIPI_DSI_TMD		/* npdc300061651 */
				pr_debug("%s: Not Change BLT", __func__);
#else	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* npdc300061651 */
				perf_req->use_ov0_blt = 1;
#endif	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* npdc300061651 */
			if (pipe->mixer_num == MDP4_MIXER1)
	...
	if( layer_change == 0 ) {
		/* bus bw calculation needlessness */
		return 0;
	}

	/* preparation of the bus bw calculation. */
	for (i = 0; i < OVERLAY_PIPE_MAX; i++, pipe++) {
		...
		/* SOLID FILL Layer check */
		solid_fill = 0;
		for(mix_num = 0; mix_num < MDP4_MIXER_MAX; mix_num++){
			for(mix_stage = 0; mix_stage < MDP4_MIXER_STAGE_MAX; mix_stage++){
				if((NULL != ctrl->blend[mix_num][mix_stage].solidfill_pipe ) &&
					(pipe->pipe_num == ctrl->blend[mix_num][mix_stage].solidfill_pipe->pipe_num) &&
					(1 == ctrl->blend[mix_num][mix_stage].solidfill)){
						solid_fill = 1;
				}
			}
		}
		if( 1 == solid_fill ) {
			solid_fill_array[i] = 1;
			continue;
		}

		/* overlap check of a layer */
		base_x_start = pipe->dst_x;
		base_x_end = pipe->dst_x + pipe->dst_w - 1;
		base_y_start = pipe->dst_y;
		base_y_end = pipe->dst_y + pipe->dst_h - 1;
		for( j=i+1, pipe2=plist+i+1; j<4; j++, pipe2++ ) {
			if(!pipe2)
				return ret;
			if(!pipe2->pipe_used)
				continue;

			over_x_start = pipe2->dst_x;
			over_x_end = pipe2->dst_x + pipe2->dst_w - 1;
			over_y_start = pipe2->dst_y;
			over_y_end = pipe2->dst_y + pipe2->dst_h - 1;

			if(!((base_y_end < over_y_start) || (base_y_start > over_y_end)) &&
				!((base_x_end < over_x_start) || (base_x_start > over_x_end))){
				overlap[i][j] = 1;
			}
		}
	}

	/* calculation of the bus bw */
	/* When the layer has overlapped, the sum total of the bus bw of each layer is computed. */
	/* No use layer or a SOLID FILL layer is the outside for bus bw calculation.             */
	for(pipe = plist, i = 0; i < 4; i++){
		if(((pipe+i)->pipe_used == 0) || (solid_fill_array[i] == 1))
			continue;

		for(j = i+1; j < 4; j++){
			if((overlap[i][j] == 1) && (solid_fill_array[j] != 1) && ((pipe+j)->pipe_used != 0)) {
				for(k = j+1; k < 4; k++){
					if((overlap[i][k] == 1) && (overlap[j][k] == 1) && (solid_fill_array[k] != 1) && ((pipe+k)->pipe_used != 0)) {
						for(l = k+1; l<4; l++) {
							if((overlap[i][l] == 1) && (overlap[k][l] == 1) && (solid_fill_array[l] != 1) && ((pipe+l)->pipe_used != 0)){
								single_bus_bw = mdp4_calc_pipe_mdp_bw_single(pipe+i)
										+ mdp4_calc_pipe_mdp_bw_single(pipe+j)
										+ mdp4_calc_pipe_mdp_bw_single(pipe+k)
										+ mdp4_calc_pipe_mdp_bw_single(pipe+l);
								if(single_bus_bw > total_bus_bw)
									total_bus_bw = single_bus_bw;
							}
						}
						single_bus_bw = mdp4_calc_pipe_mdp_bw_single(pipe+i)
								+ mdp4_calc_pipe_mdp_bw_single(pipe+j)
	...
	/* Added WriteBack & DMA_P bus bw, when BLT mode */
	if((0 != perf_req->use_ov0_blt) || (0 != perf_req->use_ov1_blt)){
		if(((total_bus_bw + ((60 * 3 * 1080 * 1920) * 2)) - ((60 * 3 * 1080 * 1920) * 2)) == total_bus_bw ){
			total_bus_bw += (60 * 3 * 1080 * 1920) * 2;
		}
		else{
	...
	perf_req->mdp_bw = mdp_get_bus_scale_level( total_bus_bw );
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
void mdp4_overlay_mdp_perf_current_set(uint32_t index)
{
	struct mdp4_overlay_perf *perf_cur = &perf_current;

	perf_cur->mdp_bw = OVERLAY_BUS_SCALE_TABLE_BASE - index;
		...
		if (perf_req->mdp_bw < perf_cur->mdp_bw) {
			...
			mdp_bus_scale_update_request
				(OVERLAY_BUS_SCALE_TABLE_BASE -
				 perf_req->mdp_bw);
#ifndef	CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
		if(( 0 == mfd->ov0_wb_buf->write_addr) && ( 0 == mfd->ov0_wb_buf->read_addr )){
#endif	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* BUS_BW_CUSTOM */
		if (perf_req->mdp_clk_rate < perf_cur->mdp_clk_rate) {
			pr_debug("%s mdp clk is changed [%d] from %d to %d\n",
				__func__,
...
#ifdef	CONFIG_FB_MSM_MIPI_DSI_TMD		/* npdc300056605 */
	/* Scaling of 5% or less of the down scaling is not carried out. */
	if(mdp4_overlay_format2type(req->src.format) == OVERLAY_TYPE_RGB) {
		/* 0x07ff is a mask value used by mdp4_overlay_req2pipe. */
		if((req->src_rect.w > req->dst_rect.w) &&
			(((req->src_rect.w & 0x07ff) * 10000) / (req->dst_rect.w & 0x07ff)) < 10050)
			req->src_rect.w = req->dst_rect.w;
		if((req->src_rect.h > req->dst_rect.h) &&
			(((req->src_rect.h & 0x07ff) * 10000) / (req->dst_rect.h & 0x07ff)) < 10050)
...
#ifdef	CONFIG_FB_MSM_MIPI_DSI_TMD		/* npdc300061651 */
	if(pipe->req_clk > mdp_max_clk){
		pr_debug("%s: mdp clk over max. src_w=%d dst_w=%d src_h=%d dst_h=%d",
				__func__, pipe->src_w, pipe->dst_w, pipe->src_h, pipe->dst_h);
		...
		pr_debug("%s: Change src/dst size. src_w=%d dst_w=%d src_h=%d dst_h=%d",
				__func__, pipe->src_w, pipe->dst_w, pipe->src_h, pipe->dst_h);
	}
#endif	/* CONFIG_FB_MSM_MIPI_DSI_TMD */        /* npdc300061651 */
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* BUS_BW_CUSTOM */
unsigned int mdp4_calc_pipe_mdp_bw_single(struct mdp4_overlay_pipe *pipe)
{
	u64 scale_rate_w = 0;
	u64 scale_rate_h = 0;
	u64 scale_rate = 0;
	...
	if((0 == pipe->src_w) || (0 == pipe->src_h) || (0 == pipe->dst_w) || ( 0 == pipe->dst_h))
		return 0;

	/* calc width scaling rate */
	val1 = pipe->src_w;
	val2 = pipe->dst_w;
	if( val1 > val2 ) {
		scale_rate_w = (val1 * 1000);
		amari = do_div(scale_rate_w, pipe->dst_w);
	}
	else {
		scale_rate_w = (val2 * 1000);
		amari = do_div(scale_rate_w, pipe->src_w);
	}

	/* calc height scaling rate */
	val1 = pipe->src_h;
	val2 = pipe->dst_h;
	if( val1 > val2 ) {
		scale_rate_h = (val1 * 1000) ;
		amari = do_div(scale_rate_h, pipe->dst_h);
	}
	else {
		scale_rate_h = (val2 * 1000);
		amari = do_div(scale_rate_h, pipe->src_h);
	}

	/* calc scaling rate */
	scale_rate = scale_rate_w * scale_rate_h;
	if( 4000000 <= scale_rate ) {
		scale_rate = 4000000;
	}

	/* upvaluation about the 3rd place of a decimal point */
        amari = do_div(scale_rate, 10000);
	if( amari != 0 )
		scale_rate += 1;
	...
	val2 = pipe->src_w;
	if( scale_rate == 100 ) {
		/* Continuation access in the same Col is 512 bytes.              */
		/* 512bytes = 16 x 32bit[MDP burst] x 8[Request pending]          */
		/* It is unnecessary to bus bw calculate 128-pixel single access. */
		/* 128bytes = 512bytes / 4(ARGB8888)                              */
		if(pipe->dst_w <= 128)
			bus_bw = 0;
		else
			bus_bw = 1080 * 1920 * 60 * val1;
	}
	else{
		bus_bw = (60 * val1 * 1920 * val2 * scale_rate);
	        amari = do_div(bus_bw , 100 );
	}

	return (unsigned int)bus_bw;
}

void mdp4_overlay_mdp_perf_req_down( struct msm_fb_data_type *mfd )
{
	mdp4_overlay_mdp_perf_req(mfd, &ctrl->plist[0]);
}

void mdp4_overlay_mdp_perf_current_set_use_ov0_blt(u32 index)
{
	struct mdp4_overlay_perf *perf_cur = &perf_current;

	perf_cur->use_ov0_blt = index;

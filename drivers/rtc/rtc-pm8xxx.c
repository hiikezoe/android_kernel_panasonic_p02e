...
/* IDP npdc300019667: Adjust kernel timer from RTC on resume */
...
static void time_adjust_late_resume(struct early_suspend *h)
{
	struct rtc_time tm;
	struct timespec ts_kernel, ts_rtc, ts_diff;

	if (!rtc_device)
		return;

	/* Read RTC and kernel time */
	pm8xxx_rtc_read_time(rtc_device, &tm);
	getnstimeofday(&ts_kernel);

#if 0
	printk(KERN_INFO "%s: called on RTC TIME %04d/%02d/%02d %02d:%02d:%02d\n", __func__,
	       tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
#endif

	/* Calculate the difference between RTC and kernel time */
	ts_rtc = ktime_to_timespec(rtc_tm_to_ktime(tm));
	ts_diff = timespec_sub(ts_rtc, ts_kernel);

	/* IDP Work-around: If 3 seconds slower or faster, set RTC time to kernel */
	if (ts_diff.tv_sec < 3 && ts_diff.tv_sec > -3) {
		printk(KERN_INFO "%s: RTC time difference %d s %ld ns\n", __func__, (int)ts_diff.tv_sec, ts_diff.tv_nsec);
	} else {
		printk(KERN_INFO "%s: RTC time difference %d s %ld ns, update kernel time\n", __func__, (int)ts_diff.tv_sec, ts_diff.tv_nsec);
		do_settimeofday(&ts_rtc);
	}
}

static struct early_suspend time_adjust_early_suspend = {
	.resume = time_adjust_late_resume
};
#endif
...
static int __devinit pm8xxx_rtc_probe(struct platform_device *pdev)
{
	...
	/* IDP npdc300019667: The compensation of RTC */
	{
/*
 * IDP npdc300035903: Fix the compensation value of RTC
 *
 * We fix the compensation value here.  We should change the EEPROM
 * value, but sig sending would be required to change it.
 */
#if 0
		unsigned char v = 67;
#else
		void *p __iomem = ioremap(DMEM_RTC_TIME_ADJUST, DMEM_RTC_TIME_ADJUST_SIZE);
		unsigned char v = readl(p) & 0xff;

		iounmap(p);
#endif
		/* Write the EEPROM value to the adjust register(0x11c) */
		rc = pm8xxx_write_wrapper(rtc_dd, &v, rtc_dd->rtc_base - 1, 1);
		if (rc < 0) {
			dev_err(&pdev->dev, "PM8xxx write failed!\n");
			goto fail_req_irq;
		}
		/* Read back to be sure and logging */
		rc = pm8xxx_read_wrapper(rtc_dd, &ctrl_reg, rtc_dd->rtc_base - 1, 1);
		if (rc < 0) {
			dev_err(&pdev->dev, "PM8xxx write failed!\n");
			goto fail_req_irq;
		}
		printk(KERN_INFO "RTC Adjust value: 0x%02x\n", ctrl_reg);
	}
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	rtc_device = &pdev->dev;
	register_early_suspend(&time_adjust_early_suspend);
#endif
...
/* IDP npdc300019667: Adjust kernel timer from RTC on resume */
#ifdef CONFIG_HAS_EARLYSUSPEND
	unregister_early_suspend(&time_adjust_early_suspend);
	rtc_device = NULL;
#endif

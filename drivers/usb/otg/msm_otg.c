...
#define USB_STS_LINK            2
#define USB_STS_CONNECT         1
#define USB_STS_NOCONNECT       0
#define USB_ENUM_COMP_CHECK_COUNT_MAX 10
#define ENUM_TIMER ( 2 * HZ)

static unsigned  int charger_detect_sdp_not_fix = 0;
static unsigned  int charger_detect_retry_cnt = 0;
static unsigned  int usb_link_stat = USB_STS_NOCONNECT;
...
static int msm_otg_suspend(struct msm_otg *motg)
{
	...
	/* IDP Work-around: Disable USB suspend for ES1 to avoid shutdown. */
...
#define MSM_CHG_DCD_MAX_RETRIES		9 /* Tdcd_tmout = 9 * 100 msec */ /* npdc300035308 */
...
static void msm_otg_sm_work(struct work_struct *w)
{
		...
		if ((!test_bit(ID, &motg->inputs) ||
			...
			case USB_CHG_STATE_DETECTED:
				...
				case USB_SDP_CHARGER:
					...
					otg->phy->state =
						OTG_STATE_B_PERIPHERAL;
					charger_detect_sdp_not_fix = 1;
					schedule_delayed_work(&motg->retry_work, ENUM_TIMER);
					break;
				default:
					break;
				}
	...
	case OTG_STATE_B_PERIPHERAL:
		if (!test_bit(ID, &motg->inputs) ||
			...
			if(charger_detect_sdp_not_fix == 1)
			{
				charger_detect_sdp_not_fix = 0;
				cancel_delayed_work_sync(&motg->retry_work);
			}
...
static void msm_otg_retry_chg_detect_work(struct work_struct *w)
{
	struct msm_otg *motg;
	struct usb_otg *otg;
	...
	{
		charger_detect_sdp_not_fix = 0;
		if(usb_link_stat == USB_STS_LINK)
		{
			dev_dbg(otg->phy->dev,"msm_otg_retry_chg_detect_work Enumeration OK\n");
		}
		else if(test_bit(B_SESS_VLD, &motg->inputs))
		{
			charger_detect_retry_cnt++;
			...
			if(charger_detect_retry_cnt <= USB_ENUM_COMP_CHECK_COUNT_MAX)
			{
...
static void msm_otg_cancel_work(struct work_struct *w)
{
	struct msm_otg *motg;
	motg = the_msm_otg;
...
/******************************************************************************/
/**
 * @fn     void USB_HS_Enumeration_Notice(void)
 * @brief  notice Enumeration complete function
 * @retval none
*/
/******************************************************************************/
void USB_HS_Enumeration_Notice(void)
{

	struct msm_otg *motg = the_msm_otg;

	if (usb_link_stat != USB_STS_LINK) {
		schedule_work(&motg->cancel_retry_work);
		usb_link_stat = USB_STS_LINK;
		charger_detect_sdp_not_fix = 0;
		charger_detect_retry_cnt = 0;
	}

	return;
}
EXPORT_SYMBOL(USB_HS_Enumeration_Notice);
...
static void msm_otg_set_vbus_state(int online)
{
	...
	if (online) {
		...
		usb_link_stat = USB_STS_CONNECT;
		set_bit(B_SESS_VLD, &motg->inputs);
	} else {
		pr_debug("PMIC: BSV clear\n");
		usb_link_stat = USB_STS_NOCONNECT;
		clear_bit(B_SESS_VLD, &motg->inputs);
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&motg->retry_work, msm_otg_retry_chg_detect_work);
	INIT_WORK(&motg->cancel_retry_work, msm_otg_cancel_work);
	charger_detect_sdp_not_fix = 0;
	charger_detect_retry_cnt = 0;

...
/* <SlimAV1> ADD START */
#define ISA1200_DUTY_MAX    127
#define ISA1200_DUTY_MIN   -127
/* <SlimAV1> ADD END */
...
/* <SlimAV1> ADD START */
struct _isa1200_data {
	struct i2c_client *client;
};

static struct _isa1200_data* isa1200_data = NULL;
/* <SlimAV1> ADD END */
struct isa1200_chip {
	struct i2c_client *client;
...
/* <SlimAV1> ADD START */
#ifdef    CONFIG_PM
static int isa1200_power_off(struct i2c_client *client, pm_message_t mesg);
static int isa1200_power_on(struct i2c_client *client);
...
{
	int value;             /* <SlimAV1> ADD */
	int reg_value;         /* <SlimAV1> ADD */
	u8  duty;              /* <SlimAV1> ADD */
	int rc = 0;

	/* <SlimAV1> ADD START */
	value = ((ISA1200_HCTRL5_VIB_STOP * 127) + (haptic->pdata->duty * 106)) / 127;
	duty =  (u8)value;
	dev_info(&haptic->client->dev, "%s duty=0x%02x\n", __func__,  duty);
	/* <SlimAV1> ADD END */
	if (enable) {
		...
		if (haptic->pdata->mode_ctrl == PWM_INPUT_MODE) {
			...
			/* <SlimAV1> ADD START */
			reg_value = haptic->pdata->pwm_mod |
				(haptic->pdata->mode_ctrl << 3) |
				(haptic->pdata->chip_en << 7);

			rc = isa1200_write_reg(haptic->client, ISA1200_HCTRL0, reg_value);
			if (rc < 0) {
				pr_err("%s: i2c write failure\n", __func__);
			}
			/* <SlimAV1> ADD END */
			rc = isa1200_write_reg(haptic->client,
						ISA1200_HCTRL5,
#if 0  /* <SlimAV1> MOD START */
						ISA1200_HCTRL5_VIB_STRT);
#else
						duty);
#endif /* <SlimAV1> MOD END */
			if (rc < 0) {
...
static void isa1200_chip_work(struct work_struct *work)
{
	...
	if ( haptic->timer_enable ) {
		if ( haptic->enable != 0) {
			isa1200_power_on( haptic->client );
		}
	}
	/* <SlimAV1> ADD END */
	...
	if ( haptic->timer_enable ) {
		if ( haptic->enable == 0) {
			mesg = PMSG_SUSPEND;
			isa1200_power_off( haptic->client, mesg );
		}
	}
	/* <SlimAV1> ADD END */
}
...
/* <SlimAV1> ADD START */
static void isa1200_chip_enable_timer_on(struct timed_output_dev *dev, int value)
{
...
/* <SlimAV1> ADD START */
static int isa1200_set_enable(bool flag)
{
	struct isa1200_chip*	haptic;
	int						ret = 0;
	pm_message_t			mesg;
	...
	pr_info("%s flag=%d\n", __func__, flag );
	if ( isa1200_data == NULL ) {
		pr_err("%s error! driver is not initialized.\n", __func__);
		ret = -ENODATA;
		goto func_end;
	}

	haptic = i2c_get_clientdata( isa1200_data->client );
	...
	if( flag ) {
		/* Power ON */
		isa1200_resume( haptic->client );
	} else {
		/* Power OFF */
		mesg = PMSG_SUSPEND;
		isa1200_suspend( haptic->client, mesg );
	}
...
{
	pr_info("%s\n", __func__);
	return isa1200_set_enable(true);
}
EXPORT_SYMBOL(isa1200_amp_enable);


int isa1200_amp_disable( void )
{
	pr_info("%s\n", __func__);
	return isa1200_set_enable(false);
}
EXPORT_SYMBOL(isa1200_amp_disable);
...
int isa1200_vibrator_control(signed char pwm_duty)
{
	...
	if( ( pwm_duty < ISA1200_DUTY_MIN ) || ( pwm_duty > ISA1200_DUTY_MAX ) ) {
	...
	if (pwm_duty == 0) {
		/* set work queue */
		isa1200_chip_enable(&haptic->dev, 0);
	}
	else {
		/* set work queue */
		isa1200_chip_enable(&haptic->dev, 1);
	}

func_end:
	return ret;
}
EXPORT_SYMBOL(isa1200_vibrator_control);
/* <SlimAV1> ADD END */
...
static void dump_isa1200_reg(char *str, struct i2c_client *client)
{
	...
	pr_debug("%s reg0x%02x=0x%02x, reg0x%02x=0x%02x, "
		...
		ISA1200_HCTRL2, isa1200_read_reg(client, ISA1200_HCTRL2),
		ISA1200_HCTRL3, isa1200_read_reg(client, ISA1200_HCTRL3),
		ISA1200_HCTRL5, isa1200_read_reg(client, ISA1200_HCTRL5),
		ISA1200_HCTRL6, isa1200_read_reg(client, ISA1200_HCTRL6),
		ISA1200_SCTRL,  isa1200_read_reg(client, ISA1200_SCTRL));
	/* <SlimAV1> MOD END */
}
...
static int isa1200_setup(struct i2c_client *client)
{
	...
	/* <SlimAV1> ADD START */
	/* Software Reset */
	value = ( haptic->pdata->hswrst_en << 7 );

	rc = isa1200_write_reg(client, ISA1200_HCTRL2, value);
	if (rc < 0) {
		goto reset_hctrl1;
	}

	/* Release Software Reset */
	value = ( haptic->pdata->hswrst_dis << 7 );
	...
	/* <SlimAV1> ADD START */
	value = haptic->pdata->pwm_per;

	rc = isa1200_write_reg(client, ISA1200_HCTRL6, value);
	if (rc < 0) {
		goto reset_hctrl1;
	}
	/* <SlimAV1> ADD END */
...
/* <SlimAV1> MOD END */
	...
	/* <SlimAV1> ADD START */
	value = haptic->pdata->pwm_high;

	rc = isa1200_write_reg(client, ISA1200_HCTRL5, value);
	if (rc < 0) {
		goto reset_hctrl1;
	}

	value =	(haptic->pdata->pplldiv << 4) | haptic->pdata->wplldiv;

	rc = isa1200_write_reg(client, ISA1200_HCTRL3, value);
	if (rc < 0) {
		goto reset_hctrl1;
	}

	value = haptic->pdata->ldo_adj;

	rc = isa1200_write_reg(client, ISA1200_SCTRL, value);
	if (rc < 0) {
		goto reset_hctrl1;
	}
	/* <SlimAV1> ADD END */
			...
			const struct i2c_device_id *id)
{
	...
	haptic->dev.enable = isa1200_chip_enable_timer_on; 	/* <SlimAV1> MOD */
/* <SlimAV1> merge from ID Power START */
#ifdef VIB_SETTING_DISABLE
...
/* <SlimAV1> merge from ID Power END */
	...
	haptic->timer_enable = false;
	haptic->power = false;
	isa1200_data = kzalloc(sizeof(struct _isa1200_data), GFP_KERNEL);
	if ( isa1200_data == NULL ) {
		dev_err(&client->dev, "%s: can not allocate isa1200_data \n", __func__);
		goto isa1200_data_fail;
	}
	isa1200_data->client = client;
	/* <SlimAV1> ADD END */

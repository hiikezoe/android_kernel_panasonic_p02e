...
static void qup_i2c_recover_bus_busy(struct qup_i2c_dev *dev)
{
	...
	uint32_t status = readl_relaxed(dev->base + QUP_I2C_STATUS);
/*	struct gpiomux_setting old_gpio_setting; *//* Quad patch */
	struct gpiomux_setting old_gpio_setting[ARRAY_SIZE(i2c_rsrcs)]; /* Quad patch */
...
/*				&recovery_config, &old_gpio_setting)) { *//* Quad patch */
				&recovery_config, &old_gpio_setting[i])) { /* Quad patch */
...
/*				&old_gpio_setting, NULL); *//* Quad patch */
				&old_gpio_setting[i], NULL); /* Quad patch */
	}
...
/* IDPower start */
	/* Chande Rate */
	if (msgs->flags & I2C_M_FORCE_SS) {
		dev->clk_ctl = 0;
		dev->pdata->clk_freq = 100000;
		msgs->flags &= ~I2C_M_FORCE_SS;
	}
	else if (msgs->flags & I2C_M_FORCE_FS) {
		dev->clk_ctl = 0;
		dev->pdata->clk_freq = 380000;
		msgs->flags &= ~I2C_M_FORCE_FS;
	}
	else{
	}
#ifdef DEBUG
  dev_dbg(dev->dev, "##Quad## FGIC I2C freq:%d \n", dev->pdata->clk_freq); /* Quad debug */
#endif
	/* printk("** dev->clk_ctl = %d, dev->pdata->clk_freq = %d\n", dev->clk_ctl, dev->pdata->clk_freq); */
/* IDPower end */

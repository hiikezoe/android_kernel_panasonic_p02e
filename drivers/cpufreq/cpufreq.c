...
#ifdef CONFIG_CPU_PAST_STAT
void cpu_past_stat_freq_rec(unsigned char cpu, unsigned int freq)
{
	struct timespec ts;
	...
	if (!cpu_past_stat_is_enable || cpu_past_stat_is_read)
		return;
	if (cpu >= CPUNUM) {
		printk("%s: error cpu=%hhu\n",__func__, cpu);
		return;
	}
	getnstimeofday(&ts);
	spin_lock_irqsave(&cpu_past_stat_freq_lock, flags);
	cpu_past_stat_freq.is_update = true;
	if (cpu_past_stat_freq.end == -1) {
		cpu_past_stat_freq.end = 0;
		ts_st = ts;
	}
	cpu_past_stat_freq.ts[cpu_past_stat_freq.end] = ts;
	cpu_past_stat_freq.cpu[cpu_past_stat_freq.end] = cpu;
	cpu_past_stat_freq.freq[cpu_past_stat_freq.end] = freq;
	cpu_past_stat_freq.end++;
	if (cpu_past_stat_freq.end >= FREQBUFFNUM) {
		cpu_past_stat_freq.end = 0;
		cpu_past_stat_freq.is_loop = true;
		printk("cpu_past_stat: frequency collect %ld -> %ld\n",
			ts_st.tv_sec, ts.tv_sec);
		ts_st = ts;
	}
	cpu_past_stat_freq.is_update = false;
	spin_unlock_irqrestore(&cpu_past_stat_freq_lock, flags);
...
#ifdef CONFIG_CPU_PAST_STAT
		cpu_past_stat_freq_rec(freqs->cpu, freqs->new);
#endif /* CONFIG_CPU_PAST_STAT */

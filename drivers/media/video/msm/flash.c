...
error:
	...
	case MSM_CAMERA_LED_LOW:
		...
		if (adp1650_client)
		{
			rc = flash_i2c_write_b(adp1650_client, 0x03, 0x02);
			rc = flash_i2c_write_b(adp1650_client, 0x04, 0xAE);
		}
		break;
	...
	case MSM_CAMERA_LED_HIGH:
		...
		if (adp1650_client)
		{
			rc = flash_i2c_write_b(adp1650_client, 0x03, 0x41);
			rc = flash_i2c_write_b(adp1650_client, 0x02, 0x02);
			rc = flash_i2c_write_b(adp1650_client, 0x04, 0xAF);
		}
		gpio_set_value_cansleep(external->led_en, 0);
		gpio_set_value_cansleep(external->led_flash_en, 0);
		break;

...
static int m_console_enable = 1;	/* npdc300034169, npdc300033557 : don't change */
/* npdc300060518 Start */
static int g_cread = 1;
/* npdc300060518 End */
...
#define IRDA_PDN				PM8921_GPIO_PM_TO_SYS(5)

#define MSM_HS_POWER_REG		0x00
#define MSM_HS_CUUR_REG			0x02

#define MSM_HS_POWER_VAL		0x04
#define MSM_HS_CUUR_ON_VAL		0x0F /* 3.3V */
#define MSM_HS_CUUR_OFF_VAL		0x09 /* Default 1.85V */
#define MSM_HS_CUUR_MASK_VAL	0xF0
#define MSM_HS_CUUR_OFF_SET_VAL	(u8)(~(MSM_HS_CUUR_OFF_VAL|MSM_HS_CUUR_MASK_VAL))

#define GSBI3_DM_BASE_REG_ADR	0x16240000
#define GSBI3_DM_BASE_REG_LEN	0x100
#define GSBI3_DM_IRDA_ADR		0x38
#define GSBI3_DM_MODE_NORMAL	0x00
#define GSBI3_DM_MODE_IRDA		0x03

static char *gsbi3_dm;
struct wake_lock irda_wake_lock;

#define PMC_IRDA_DEBUG 0

#if PMC_IRDA_DEBUG
#define INFO_PRINT(arg, ...)				printk(KERN_INFO "[Serial_hsl]:" arg, ##__VA_ARGS__)
#define DEBUG_PRINT(arg, ...)				printk(KERN_DEBUG "[Serial_hsl]:" arg, ##__VA_ARGS__)
#else
#define INFO_PRINT(arg, ...)
#define DEBUG_PRINT(fmt, ...)
#endif
#define DEBUG_PRINT_ERR(arg, ...)			printk(KERN_ERR "[Serial_hsl]:" arg, ##__VA_ARGS__)
/*Quad IrDA ADD END */
...
/*Quad IrDA ADD START */
static int32_t msm_serial_hsl_power_on(void)
{
	int32_t ret = 0;
	struct regulator *vpro_vreg_l21;
	struct regulator *vpro_vreg_l17;

	INFO_PRINT("%s [IN]\n",__func__);

	gsbi3_dm = ioremap_nocache(GSBI3_DM_BASE_REG_ADR,GSBI3_DM_BASE_REG_LEN);
	if (gsbi3_dm == NULL) {
		DEBUG_PRINT_ERR("%s: ioremap_nocache error\n",__func__);
		return -1;
	}
	iowrite32(GSBI3_DM_MODE_IRDA,gsbi3_dm+GSBI3_DM_IRDA_ADR);

	mb();
	iowrite32(0x1,gsbi3_dm+0x10);
	mb(); 

	mdelay(1);
/*Quad IrDA MOD START */
	DEBUG_PRINT("%s: addr[0x%x],val[0x%x],mode[0x%x]\n",__func__,
										MSM_HS_CUUR_REG,
										MSM_HS_CUUR_ON_VAL,
										SUBPMIC_BITSET_ON);
	ret = subpmic_i2c_bitset_u8( MSM_HS_CUUR_REG, MSM_HS_CUUR_ON_VAL, SUBPMIC_BITSET_ON);
	if (ret < 0 ) {
		DEBUG_PRINT_ERR("%s: i2c write error\n",__func__);
		return ret;
	}
	mdelay(5);
/*Quad IrDA MOD START */
	DEBUG_PRINT("%s: addr[0x%x],val[0x%x],mode[0x%x]\n",__func__,
										...
										MSM_HS_POWER_VAL,
										SUBPMIC_BITSET_ON);
	ret = subpmic_i2c_bitset_u8( MSM_HS_POWER_REG, MSM_HS_POWER_VAL, SUBPMIC_BITSET_ON);
/*Quad IrDA MOD END */
	if (ret < 0 ) {
		DEBUG_PRINT_ERR("%s: i2c write error\n",__func__);
		return ret;
	...
	mdelay(5);
	vpro_vreg_l17 = regulator_get(NULL, "8921_l17");
	if (IS_ERR(vpro_vreg_l17)) {
		DEBUG_PRINT_ERR("%s: Failed (%ld)\n",__func__,PTR_ERR(vpro_vreg_l17));
/*Quad IrDA ADD START */
		ret = -1;
/*Quad IrDA ADD END */
		goto an30183a_err;
	}
	DEBUG_PRINT("%s: vpro_vreg_l17 OK\n",__func__);
	regulator_set_voltage(vpro_vreg_l17, 2800000,2800000);
	regulator_enable(vpro_vreg_l17);
	mdelay(1);

	vpro_vreg_l21 = regulator_get(NULL, "8921_l21");
	if (IS_ERR(vpro_vreg_l21)) {
		DEBUG_PRINT_ERR("%s: Failed (%ld)\n",__func__,PTR_ERR(vpro_vreg_l21));
/*Quad IrDA ADD START */
		ret = -1;
/*Quad IrDA ADD END */
		goto pm8921_l17_err;
	}
	DEBUG_PRINT("%s: vpro_vreg_l21 OK\n",__func__);
	regulator_set_voltage(vpro_vreg_l21, 1800000,1800000);
	regulator_enable(vpro_vreg_l21);
	mdelay(1);	

	gpio_set_value_cansleep(IRDA_PDN, 1);
	ret = gpio_get_value(IRDA_PDN);
	DEBUG_PRINT("%s: GPIO(%d) gpio_ret[%d]\n",__func__,IRDA_PDN,ret);
	if (ret < 0) {
		goto pm8921_l21_err;
	}
	mdelay(1);

	gpio_set_value_cansleep(IRDA_PDN, 0);
	ret = gpio_get_value(IRDA_PDN);
	DEBUG_PRINT("%s: GPIO(%d) gpio_ret[%d]\n",__func__,IRDA_PDN,ret);

	INFO_PRINT("%s [OUT] ret[%d] \n",__func__,ret);
	return ret;

pm8921_l21_err:
	regulator_disable(vpro_vreg_l21);
/*Quad IrDA ADD START */
	regulator_put(vpro_vreg_l21);
/*Quad IrDA ADD END */
	mdelay(1);
	DEBUG_PRINT("pm8921_l21_err \n");
	DEBUG_PRINT("vreg_l21 desable \n");
pm8921_l17_err:
	regulator_disable(vpro_vreg_l17);
/*Quad IrDA ADD START */
	regulator_put(vpro_vreg_l17);
/*Quad IrDA ADD END */
	mdelay(1);
	DEBUG_PRINT("pm8921_l17_err \n");
	DEBUG_PRINT("vreg_l17 desable \n");
an30183a_err:
/*Quad IrDA MOD START */
	subpmic_i2c_bitset_u8( MSM_HS_POWER_REG, MSM_HS_POWER_VAL, SUBPMIC_BITSET_OFF);
/*Quad IrDA MOD END */
/*Quad IrDA BUG:xxx START */
	subpmic_i2c_bitset_u8( MSM_HS_CUUR_REG, MSM_HS_CUUR_OFF_SET_VAL, SUBPMIC_BITSET_OFF);
/*Quad IrDA BUG:xxx END */
	mdelay(1);
	DEBUG_PRINT("an30183a_err \n");
	DEBUG_PRINT("an30183a power off \n");
	return ret;
}

static int32_t msm_serial_hsl_power_off(void)
{
	int32_t ret = 0;
	struct regulator *vpro_vreg_l21;
	struct regulator *vpro_vreg_l17;
	...
	iowrite32(GSBI3_DM_MODE_NORMAL,gsbi3_dm+GSBI3_DM_IRDA_ADR);
	iounmap(gsbi3_dm);
...
/*Quad IrDA MOD START */
	DEBUG_PRINT("%s: addr[0x%x],val[0x%x],mode[0x%x]\n",__func__,
										...
										MSM_HS_POWER_VAL,
										SUBPMIC_BITSET_OFF);
	ret = subpmic_i2c_bitset_u8( MSM_HS_POWER_REG, MSM_HS_POWER_VAL, SUBPMIC_BITSET_OFF);
/*Quad IrDA MOD END */
	if (ret < 0 ) {
		DEBUG_PRINT_ERR("%s: i2c write error\n",__func__);
	}
/*Quad IrDA MOD START */
	DEBUG_PRINT("%s: addr[0x%x],val[0x%x],mode[0x%x]\n",__func__,
										MSM_HS_CUUR_REG,
										MSM_HS_CUUR_OFF_SET_VAL,
										SUBPMIC_BITSET_OFF);
	ret = subpmic_i2c_bitset_u8( MSM_HS_CUUR_REG, MSM_HS_CUUR_OFF_SET_VAL, SUBPMIC_BITSET_OFF);
/*Quad IrDA MOD END */
	if (ret < 0 ) {
		DEBUG_PRINT_ERR("%s: i2c write error\n",__func__);
	}
...
/* npdc300060518 Start */
	if(port->line == 1) {
		/* enable TX & RX */
		msm_hsl_write(port, UARTDM_CR_TX_EN_BMSK|UARTDM_CR_RX_EN_BMSK, regmap[msm_hsl_port->ver_id][UARTDM_CR]);
		DEBUG_PRINT("%s : RX_ENABLE \n",__func__);
	}
/* npdc300060518 End */
}
...
/* npdc300060518 Start */
	if(port->line == 1) {
		...
		msm_hsl_write(port, UARTDM_CR_TX_EN_BMSK|UARTDM_CR_RX_DISABLE_BMSK, regmap[msm_hsl_port->ver_id][UARTDM_CR]);
		DEBUG_PRINT("%s : RX_DISABLE \n",__func__);
	}
/* npdc300060518 End */
...
static void handle_rx(struct uart_port *port, unsigned int misr)
{
	...
	DEBUG_PRINT("%s: count[%d]line[%d]\n",__func__,count,port->line);
...
static void handle_tx(struct uart_port *port)
{
	...
	while (tf_pointer < tx_count)  {
		...
		DEBUG_PRINT("%s: writedata[0x%08x]\n",__func__,x);
...
/* npdc300060518 Start */
	if (port->line != 1 ) {
		...
		if(g_cread  == 1) {
			...
			data = UARTDM_CR_TX_EN_BMSK;
			data |= UARTDM_CR_RX_DISABLE_BMSK;
			/* enable TX & disable RX */
			msm_hsl_write(port, data, regmap[vid][UARTDM_CR]);
		}
	}
...
/*Quad IrDA ADD START */
	if (port->line == 1) {
		DEBUG_PRINT("%s : wake_lock \n",__func__);
		wake_lock(&irda_wake_lock);
		ret = msm_serial_hsl_power_on();
		if(ret < 0){
			pr_err("%s: power on failed for:%d\n",
						__func__, ret);
			wake_unlock(&irda_wake_lock);
			return ret;
		}
	}
/*Quad IrDA ADD END */
...
/*Quad IrDA ADD START */
    ...
    DEBUG_PRINT("%s : [IN] \n",__func__);
	if (port->line == 1) {
		ret = msm_serial_hsl_power_off();
		if(ret < 0){
			printk(KERN_ERR "%s failed to msm_serial_hsl_power_off\n", __FUNCTION__);
		}
		DEBUG_PRINT("%s : wake_unlock \n",__func__);
		wake_unlock(&irda_wake_lock);
	}
/*Quad IrDA ADD END */
...
/* Quad add start */
static struct gpiomux_setting gsbi4_uart_tx_config = {
	.func = GPIOMUX_FUNC_1,
	.drv = GPIOMUX_DRV_8MA,
	.pull = GPIOMUX_PULL_NONE,
};

static struct gpiomux_setting gsbi4_uart_rx_config = {
	.func = GPIOMUX_FUNC_1,
	.drv = GPIOMUX_DRV_8MA,
	.pull = GPIOMUX_PULL_NONE,
};
...
static struct msm_gpiomux_config apq8064_gsbi_configs[] /*__initdata*/ = {
	{
		...
		.settings = {
			[GPIOMUX_SUSPENDED] = &gsbi4_uart_tx_config,
			[GPIOMUX_ACTIVE] = &gsbi4_uart_tx_config,
		},
	},
	{
		.gpio      = 11,		/* GSBI4 UART RX */
		.settings = {
			[GPIOMUX_SUSPENDED] = &gsbi4_uart_rx_config,
			[GPIOMUX_ACTIVE] = &gsbi4_uart_rx_config,
		},
	},
};
/* Quad add end */
...
/* Quad add start */
	if(enable){
		pr_info("%s(): Set UART config for GPIO10/11\n", __func__);
		gpio_tlmm_config(GPIO_CFG(10, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, 0), GPIO_CFG_ENABLE);
		gpio_tlmm_config(GPIO_CFG(11, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, 0), GPIO_CFG_ENABLE);
		msm_gpiomux_install(apq8064_gsbi_configs, ARRAY_SIZE(apq8064_gsbi_configs));
	}
/* Quad add end */
	...
	case 0:
		...
		m_console_enable = 0;	/* npdc300034169,npdc300033557 */
		break;
	case 1:
		...
		m_console_enable = 1;	/* npdc300034169,npdc300033557 */
		break;
	default:
		return -EINVAL;
	}
...
/*Quad IrDA ADD START */
	if (port->line == 1 ) {
		DEBUG_PRINT("%s : wake_lock_init \n",__func__);
		wake_lock_init(&irda_wake_lock, WAKE_LOCK_SUSPEND, "irda_wake_lock");
	}
/*Quad IrDA ADD END */
	return ret;
}
...
static int __devexit msm_serial_hsl_remove(struct platform_device *pdev)
{
	...
	if (port->line == 1 ) {
		DEBUG_PRINT("%s : wake_lock_destroy \n",__func__);
		wake_lock_destroy(&irda_wake_lock);
	}
	return 0;
}

...
unsigned char anti_comm_name[MAX_ANTI_COMM_NUMS][MAX_ONE_CHARACTERSIZE];
unsigned char anti_comm_nums = CCS_FILE_ERROR;
...
static void ccs_file_open_anti(void)
{
	int retlen;
	struct file *fp;
	char *filename = "/sbin/ccs-file";
	unsigned char i = 0;
	...
	if (IS_ERR(fp)) {
		anti_comm_nums = CCS_FILE_ERROR;
		return;
	}

	buf = kmalloc(MAX_ALL_CHARACTERSIZE * sizeof(char), CCS_GFP_FLAGS);
	if(!buf)
	{
		anti_comm_nums = CCS_FILE_ERROR;
		filp_close(fp, NULL);
	...
	memset(anti_comm_name, 0, MAX_ALL_CHARACTERSIZE);
	memset(buf, 0, MAX_ALL_CHARACTERSIZE);

	retlen = kernel_read(fp, 0, buf, MAX_ALL_CHARACTERSIZE);
	if(strncmp((buf + retlen -1),"\n",1))
	{
		if(retlen == MAX_ALL_CHARACTERSIZE)
		{
			anti_comm_nums = CCS_FILE_ERROR;
			filp_close(fp, NULL);
			kfree(buf);
	...
	for(i=0; i<retlen; i++)
	{
		if(!strncmp((buf+i),"\n",1))
		{
			anti_comm_name[nums][characters] = '\0';
			characters = 0;
			nums++;
			if(nums > MAX_ANTI_COMM_NUMS)
			{
				anti_comm_nums = CCS_FILE_ERROR;
				filp_close(fp, NULL);
				kfree(buf);
		...
		{
			anti_comm_name[nums][characters] = *(buf + i);
			characters++;
			if(characters > (MAX_ONE_CHARACTERSIZE - 1) )
			{
				anti_comm_nums = CCS_FILE_ERROR;
				filp_close(fp, NULL);
				kfree(buf);

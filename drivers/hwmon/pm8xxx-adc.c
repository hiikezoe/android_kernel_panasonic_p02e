...
#include <linux/laputa-fixaddr.h>    /* IDPower npdc300034410 */
#include <linux/io.h>                /* IDPower npdc300034410 */
...
static uint32_t pm8xxx_adc_btm_read(uint32_t channel)
{
	...
	if (pmic_adc->batt.btm_warm_fn != NULL){
		...
		enable_irq_wake(adc_pmic->btm_warm_irq);  /* IDPower warm_cool */
	}

	if (pmic_adc->batt.btm_cool_fn != NULL){
		enable_irq(adc_pmic->btm_cool_irq);
		enable_irq_wake(adc_pmic->btm_cool_irq);  /* IDPower warm_cool */
	}
...
static int __devinit pm8xxx_adc_probe(struct platform_device *pdev)
{
	...
	int rc = 0;
	unsigned int *version_val = NULL;    /* IDPower npdc300034410 */
	int btm_value;                       /* IDPower npdc300034410 */
...
/* IDPower npdc300034410 start */
	version_val = ioremap(DMEM_CHG_AD_PCB_VERSION, DMEM_CHG_AD_PCB_VERSION_SIZE);
	if(((*version_val) & 0x10) == 0x10){
		btm_value = 1;
	}
	else{
		btm_value = 0;
	}
	pm8xxx_adc_set_btm_threshold(btm_value);

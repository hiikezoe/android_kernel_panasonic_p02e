...
#define	NODE_MAJOR			82	/* Major No. */	//	2011/05/11 Mod
...
#define       PRINT_DEBUG             KERN_DEBUG	//	2011/05/11 Mod
...
#define	AN30183A_I2C_ADDRESS			0x6E    /* I2C6 powerIC AN30183A SA:0x10 */
#define MB86A35_PADCONF_GPIO_OE_ADDRESS		0x48057134	/* Add ：(BugLister ID96) 2012/01/10 */
#define MB86A35_PADCONF_GPIO88_ADDRESS		0x4A1000C8	/* Add ：(BugLister ID98) 2012/01/12 */
#define	MB86A35_I2C_ADAPTER_ID			7	/* connected with I2C ch6 (bus id is 1) */
#define	AN30183A_I2C_ADAPTER_ID			7	/* connected with I2C ch6 (bus id is 1) */
#define	MB86A35_I2C_RFICADDRESS			( 0xC2 >> 1 )
#define D_GPIO_NDTV_RST				153	/* nDTV_RST PMIC GPIO2 */
#define D_GPIO_OUT_HI				1
#define D_GPIO_OUT_LO				0


#define	MB86A35_SELECT_UHF			1
#define	MB86A35_SELECT_VHF			0

#define MB86A35_CN_MONI_WAITTIME(n)		(( 1260 * ( 2 << ( (n) + 1 )))/1000 + 10)	//	2011/07/05 Mod(RuleC)

#define MB86A35_MER_MONI_WAITTIME(n)		(( 1260 * ( 2 << (n) ))/1000 + 10)		//	2011/07/05 Mod(RuleC)
...
static inline char SEGCNT_FROM_CHNO(int chno)
{
	...
	return SEGCNT_T[chno];
									...
									/* SEGCNT */
									/* ISDB-Tmm / Tsb */
#define	PARAM_FTSEGCNT_SEGCNT_ISDB_Tmm(n)		(SEGCNT_FROM_CHNO(n))	//	2011/07/05 Mod (RuleC)
...
MB86A35_RF_init_UHF_t RF_INIT_UHF_DATA1[] = {
	...
	{0x60, 0x49}, {0x61, 0x50}, {0x62, 0x96},	/* Sysdes Ver1.0 2012/05/23 */
	{0x63, 0x96},               {0x65, 0xff},
	{0x66, 0xff},               {0x68, 0x90},
...
MB86A35_RF_init_UHF_t RF_INIT_VHF_DATA1[] = {
	...
	{0x58, 0xdf},               {0x5a, 0x06},
	              {0x5c, 0x42}, {0x5d, 0x1c},
	{0x5e, 0x2a}, {0x5f, 0x22}, {0x60, 0x49},	/* Sysdes Ver1.0 2012/05/23 */
...
MB86A35_RF_init_UHF_t RF_INIT_UHF_DATA2[] = {
	...
	{0x60, 0x49}, {0x61, 0x50}, {0x62, 0x51},	/* Sysdes Ver1.0 2012/05/23 */
	{0x63, 0xd4},               {0x65, 0xff},
	{0x66, 0xff},               {0x68, 0x90},
...
MB86A35_RF_init_UHF_t RF_INIT_VHF_DATA2[] = {
	...
	{0x58, 0xdf},               {0x5a, 0x04},
	              {0x5c, 0x02}, {0x5d, 0x1c},
	{0x5e, 0x2a}, {0x5f, 0x32}, {0x60, 0x49},	/* Sysdes Ver1.0 2012/05/23 */
...
struct mb86a35_cmdcontrol {
	...
	ioctl_powerctrl_t POWERCTRL; 	/* Mod ：(BugLister ID14) 2011/09/06 */
	ioctl_pipe_t PIPE;				/* Add ：(BugLister ID52) 2011/09/15 */
	ioctl_irq_tuner_t IRQ_TUNER;	/* Add ：(BugLister ID52) 2011/09/15 */
	ioctl_ts_start_t TS_START;		/* Add ：(BugLister ID86) 2011/11/07 */
	ioctl_get_initial_value_mm_t INITIAL_VALUE_MM;/* Add : (Buglister ID94) 2011/12/12 */
	ioctl_get_initial_value_dtv_t INITIAL_VALUE_DTV;/* Add : (Buglister ID95) 2011/12/15 */
	ioctl_set_tuner_user_t SET_TUNER_USER;		/* Add 2012/05/21*/

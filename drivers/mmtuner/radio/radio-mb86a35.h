...
#include <string.h>
#endif
/* Mod Start：(BugLister ID86) 2011/11/01 */
#define	NODE_PATHNAME		"/dev/mmtuner"	/* Node Name */
#define	NODE_DEVNAME		"mmtuner"		/* Node Device Name */
/* Mod End  ：(BugLister ID86) 2011/11/01 */
#define	PIPENAMELEN			2048			/* Add ：(BugLister ID52) 2011/09/15 */
...
//	2011/05/25 Mod Start
	IOCTL_TS_START_RECV,	/* Receive TS  : Start Receiving TS */
	IOCTL_TS_STOP_RECV,		/* Receive TS  : Stop Receiving TS */
//	2011/05/25 Mod End
	...
	IOCTL_SET_PIPE,				/* IRQ : SET PIPE NAME */ 
	IOCTL_IRQ_TUNER_SET,		/* IRQ : SET TUNER IRQ */ 
	/* Add End  ：(BugLister ID52) 2011/09/15 */
	IOCTL_CLOSE_PIPE,		/* IRQ : CLOSE PIPE *//* Add 2012/05/22 */
	/* Add Start：(BugLister ID94) 2011/12/12 */
	IOCTL_GET_INITIAL_VALUE_MM,		/* GET:INITIAL VALUE(MM) */
	/* Add End  ：(BugLister ID94) 2011/12/12 */
	/* Add Start：(BugLister ID95) 2011/12/14 */
	IOCTL_GET_INITIAL_VALUE_DTV,		/* GET:INITIAL VALUE(DTV) */
	/* Add End  ：(BugLister ID95) 2011/12/14 */
	IOCTL_CN_MONI = 0x2001,	/* C/N Monitor : C/N Monitoring */
	/* Add Start：(BugLister ID48) 2011/09/20 */
	IOCTL_CN_MONISTART,		/* C/N Monitor : Init C/N Monitoring */
	IOCTL_CN_MONIGET,		/* C/N Monitor : Get C/N Monitoring */
	/* Add End  ：(BugLister ID48) 2011/09/20 */

	IOCTL_MER_MONI = 0x4001,	/* MER Monitor : MER Monitoring */
	/* Add Start：(BugLister ID48) 2011/09/20 */
	IOCTL_MER_MONISTART,		/* MER Monitor : Init MER Monitoring */
	IOCTL_MER_MONIGET,			/* MER Monitor : Get MER Monitoring */
	/* Add End  ：(BugLister ID48) 2011/09/20 */
	...
	IOCTL_SET_TUNER_USER = 0x3000,  /* TUNER USER : tuner user setting */
...
/* Mod Start：(BugLister ID14) 2011/09/07 */
							/* POWERCTRL */
#define		PARAM_POWER_ON			0x01	/* power on */
#define		PARAM_POWER_OFF			0x00	/* power off */
/* Mod End：(BugLister ID14) 2011/09/07 */
...
#define		PARAM_MACRO_PDOWN_DACPWDN_ON	0x20	/* Power-down */
#define		PARAM_MACRO_PDOWN_ADCPWDN_ON	0x02	/* ADC Power-down *//* Add：(BugLister ID86) 2011/11/01 */
#define		PARAM_MACRO_PDOWN_ALL_ON		0x33	/* ALL Power-down *//* Add：(BugLister ID86) 2011/11/01 */
...
#define		PARAM_GPIO_DAT_0		0x01		/* GPIO-DAT 0x01 *//* Add：(BugLister ID86) 2011/11/07 */
#define		PARAM_GPIO_DAT_CLEAR	0x00		/* GPIO-DAT 0x00 *//* Add：(BugLister ID86) 2011/11/07 */
...
/* Add Start：(BugLister ID52) 2011/09/15 */
#define		PARAM_TUNER_IRQ_DISABLE_WOHD	0x05	/* Disable Tuner IRQ(without Handler) *//* Mod：(BugLister ID90) 2011/11/29 */
#define		PARAM_TUNER_IRQ_ENABLE_WOHD		0x04	/* Enable Tuner IRQ(without Handler) *//* Mod：(BugLister ID90) 2011/11/29 */
#define		PARAM_TUNER_IRQ_DELETE			0x03	/* Delete Tuner IRQ *//* Add：(BugLister ID86) 2011/11/01 */
#define		PARAM_TUNER_IRQ_INIT			0x02	/* Init Tuner IRQ *//* Add：(BugLister ID70) 2011/10/17 */
#define		PARAM_TUNER_IRQ_ENABLE			0x01	/* Enable Tuner IRQ */
#define		PARAM_TUNER_IRQ_DISABLE			0x00	/* Disable Tuner IRQ */
/* Add End  ：(BugLister ID52) 2011/09/15 */
/* Add Start：(BugLister ID86) 2011/11/07 */
#define		PARAM_TS_RS_ENABLE					0x00	/* Enable TS with RS Packet */
#define		PARAM_TS_RS_DISABLE					0x01	/* Disable TS with RS Packet */
#define		PARAM_TS_RS_ENABLE_RESET_DISABLE	0x02	/* Enable TS with RS Packet & Disable SyncError Reset *//* Mod：(MMシステム対応) 2012/03/14 */
...
#define		PARAM_USER_DTV		0x00	/* USER DTV */
#define		PARAM_USER_MM		0x01    /* USER MM */
...
static u32 inline GET32(u32 * merdata)
{
	u32 rtncode = 0;
...
struct ioctl_powerctrl{
	u8 power;         /* power mode  */
};
typedef struct ioctl_powerctrl ioctl_powerctrl_t;
/* Mod End：(BugLister ID14) 2011/09/06 */
/* Add Start：(BugLister ID52) 2011/09/15 */
struct ioctl_pipe {
	u8 pipename[PIPENAMELEN];		/* pipe name */
};
typedef struct ioctl_pipe ioctl_pipe_t;

struct ioctl_irq_tuner {
	u8 tunerirq;		/* tuner irq setting */
};
typedef struct ioctl_irq_tuner ioctl_irq_tuner_t;
/* Add End  ：(BugLister ID52) 2011/09/15 */
/* Add Start：(BugLister ID86) 2011/11/07 */
struct ioctl_ts_start {
	u8 rspacket;		/* RS Packet Setting */
};
typedef struct ioctl_ts_start ioctl_ts_start_t;
/* Add End  ：(BugLister ID86) 2011/11/07 */

/* Add Start：(BugLister ID94) 2011/12/12 */
struct ioctl_get_initial_value_mm_pict {
	u32 an_qpsk0_1;        /*  Pict Thr,QPSK,R=2/3 (0<->1)*/
	u32 an_qpsk1_2;        /*  Pict Thr,QPSK,R=2/3 (1<->2)*/
	u32 an_qpsk2_3;        /*  Pict Thr,QPSK,R=2/3（2<->3)*/
	u32 an_qpsk3_4;        /*  Pict Thr,QPSK,R=2/3（3<->4)*/
	u32 an_qpsk4_5;        /*  Pict Thr,QPSK,R=2/3（4<->5)*/
	u32 an_16qam0_1;       /*  Pict Thr,16QAM,R=1/2(0<->1)*/
	u32 an_16qam1_2;       /*  Pict Thr,16QAM,R=1/2(1<->2)*/
	u32 an_16qam2_3;       /*  Pict Thr,16QAM,R=1/2(2<->3)*/
	u32 an_16qam3_4;       /*  Pict Thr,16QAM,R=1/2(3<->4)*/
	u32 an_16qam4_5;       /*  Pict Thr,16QAM,R=1/2(4<->5)*/
};
typedef struct ioctl_get_initial_value_mm_pict ioctl_get_initial_value_mm_pict_t; 

struct ioctl_get_initial_value_mm_unvol2 {
	u16  mer_wait;          /* Stable time for Mer */
	u8   mer_cycle;         /* Wait time for Mer */
	u8   ber_cycle2;        /* Ber Cycle */
	u8   rssi_wait;         /* Stable time for RSSI */
	u8   rssi_cycle;        /* RSSI Cycle*/
	u8   init_ant_flag;     /* Initial Antenna */
	u8   out_flag;          /* Flag for switching antenna */
	u8   sync_cycle;        /* Interval for Watching Sync State */
	u8   in_to_out_thr;     /* Switching time (Inner -> out)*/
	u8   out_to_in_thr;     /* Switching time (out -> Inner)*/
/*	u8   dummy;*/             /* dummy */
};
typedef struct ioctl_get_initial_value_mm_unvol2 ioctl_get_initial_value_mm_unvol2_t;

struct ioctl_get_initial_value_mm {
    ioctl_get_initial_value_mm_pict_t antenna_pict;
    ioctl_get_initial_value_mm_unvol2_t unvol_dat2;
};
typedef struct ioctl_get_initial_value_mm ioctl_get_initial_value_mm_t;

#define INITIAL_VALUE_MM_UNVOL2_SIZE 11

/* Add End  ：(BugLister ID94) 2011/12/12 */

/* Add Start：(BugLister ID95) 2011/12/14 */
struct ioctl_get_initial_value_dtv_unvol {
	u16 wait_tmcc;           /*  Timer Value for reading tmccdata */
	u16 cn_wait;             /*  stable time for CN */
	u8  cn_cycle;            /*  CN Cycle */
	u8  agc_wait;            /*  agc_wait */
	u8  agc_cycle;           /*  cycle agc */
	u8  ber_wait;            /*  stable time for BER */
	u8  ber_cylce;           /*  BER cycle */
	u8  state_moniter_cycle; /*  state monitor cylce */
	u8  wait_search_init;    /*  timer value for search interrupt */
	u8  wait_mnt_sync;       /*  wait time for syncstate */
	u8  agc_thr;             /*  agc threshhold */
	u8  search_time;         /*  search time    */
	u8  dummy1;              /*  dummy */
	u8  dummy2;              /*  dummy */
};
typedef struct ioctl_get_initial_value_dtv_unvol ioctl_get_initial_value_dtv_unvol_t; 

struct ioctl_get_initial_value_dtv_pict {
	u32 cn_qpsk1_2_M;        /*  Pict Thr,QPSK,R=1/2 (0<->1)*/
	u32 cn_qpsk1_2_H;        /*  Pict Thr,QPSK,R=1/2 (1<->2)*/
	u32 cn_qpsk2_3_M;        /*  Pict Thr,QPSK,R=2/3（0<->1)*/
	u32 cn_qpsk2_3_H;        /*  Pict Thr,QPSK,R=2/3（1<->2)*/
	u32 cn_16qam1_2_M;       /*  Pict Thr,16QAM,R=1/3（0<->1)*/
	u32 cn_16qam1_2_H;       /*  Pict Thr,16QAM,R=1/2 (1<->2)*/
};
typedef struct ioctl_get_initial_value_dtv_pict ioctl_get_initial_value_dtv_pict_t; 

/* Add Start：(DTV CN値平滑化用の不揮発追加) 2012/04/11 */
struct ioctl_get_initial_value_dtv_cn {
	u16 cn_wait;             /* stable time for CN     */
	u16 cn_ave_count;        /* stable count for CN    */
	u8  cn_cycle;            /* CN Cycle               */
	u8  cn_alpha;            /* calc parameter1 for CN */
	u8  cn_beta;             /* calc parameter2 for CN */
	u8  cn_gamma;            /* calc parameter3 for CN */
};
typedef struct ioctl_get_initial_value_dtv_cn ioctl_get_initial_value_dtv_cn_t; 
/* Add End  ：(DTV CN値平滑化用の不揮発追加) 2012/04/11 */

struct ioctl_get_initial_value_dtv_unvol2 {                                    /* 20120626 0.51版対応  */
	u8  init_ant_flag;                                                         /* 20120626 0.51版対応 初期アンテナ選択フラグ */
	u8  ant_switch_flag;                                                       /* 20120626 0.51版対応 外部アンテナ切替え制御実施フラグ */
	u8  switch_ant_cycle;                                                      /* 20120626 0.51版対応 アンテナ切替え用State監視周期 */
	u8  external_ant_threshold;                                                /* 20120626 0.51版対応 外部アンテナ切替え閾値 */
	u8  internal_ant_threshold;                                                /* 20120626 0.51版対応 内部アンテナ切替え閾値 */
};                                                                             /* 20120626 0.51版対応 */
typedef struct ioctl_get_initial_value_dtv_unvol2 ioctl_get_initial_value_dtv_unvol2_t; /* 20120626 0.51版対応 */

struct ioctl_get_initial_value_dtv_ant{
    u32 cn_qpsk1_2_ant;
    u32 cn_qpsk2_3_ant;
    u32 cn_16qam1_2_ant;
}; 
typedef struct ioctl_get_initial_value_dtv_ant ioctl_get_initial_value_dtv_ant_t;

struct ioctl_get_initial_value_dtv {
    ioctl_get_initial_value_dtv_unvol_t unvol_dat;
    ioctl_get_initial_value_dtv_pict_t antenna_pict;
    ioctl_get_initial_value_dtv_cn_t cn_dat;	/* Add：(DTV CN値平滑化用の不揮発追加) 2012/04/11 */
    ioctl_get_initial_value_dtv_unvol2_t unvol_dat2;                           /* 20120626 0.51版対応 */
    ioctl_get_initial_value_dtv_ant_t antenna_select;
};
typedef struct ioctl_get_initial_value_dtv ioctl_get_initial_value_dtv_t;

#define INITIAL_VALUE_DTV_UNVOL_SIZE 14
#define INITIAL_VALUE_DTV_ANT_SIZE 12

struct ioctl_set_tuner_user {
	u32 tuner_user;
};
typedef struct ioctl_set_tuner_user ioctl_set_tuner_user_t;

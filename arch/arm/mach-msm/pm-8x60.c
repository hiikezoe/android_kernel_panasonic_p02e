...
extern unsigned int grupy_test_flg; //<noise_patch>
/*
		...
		struct cpuidle_driver *drv, int index)
{
	...
	for (i = 0; i < dev->state_count; i++) {
		...
		if (dev->cpu == 0 && grupy_test_flg && mode != MSM_PM_SLEEP_MODE_WAIT_FOR_INTERRUPT) { //<noise_patch>
			allow = false; //<noise_patch>
		} else { //<noise_patch>
...
static int msm_pm_enter(suspend_state_t state)
{
	...
	for (i = 0; i < MSM_PM_SLEEP_MODE_NR; i++) {
		...
		if (grupy_test_flg && mode != MSM_PM_SLEEP_MODE_WAIT_FOR_INTERRUPT) { //<noise_patch>
			allow[i] = false; //<noise_patch>
		} else { //<noise_patch>
		allow[i] = mode->suspend_supported && mode->suspend_enabled;
		} //<noise_patch>
	}

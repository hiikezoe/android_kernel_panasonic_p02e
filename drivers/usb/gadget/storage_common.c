			     ...
			     char *buf)
{
	...
	ssize_t		rc;
	FUNC_IN();
	CPRM_LOGOUT("down_read(%p) in %s(line:%d)", filesem, __FILE__, __LINE__);
	down_read(filesem);
	...
	CPRM_LOGOUT("up_read(%p) in %s(line:%d)", filesem, __FILE__, __LINE__);
	up_read(filesem);
	FUNC_OUT();
	return rc;
}
			      ...
			      const char *buf, size_t count)
{
	...
	CPRM_LOGOUT("down_write(%p) in %s(line:%d)", filesem, __FILE__, __LINE__);
	down_write(filesem);
	if (fsg_lun_is_open(curlun)) {
	...
	CPRM_LOGOUT("up_write(%p) in %s(line:%d)", filesem, __FILE__, __LINE__);
	up_write(filesem);
// merge s
    notify_change_path((char*)buf);
// merge e
	FUNC_OUT();

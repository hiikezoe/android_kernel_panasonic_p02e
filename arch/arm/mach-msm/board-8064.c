...
//rupy DEL #include <linux/i2c/isa1200.h>
#include <linux/i2c/drv2604.h> //rupy ADD
...
//rupy DEL #define HAP_SHIFT_LVL_OE_GPIO		PM8921_MPP_PM_TO_SYS(8)
//rupy DEL #define ISA1200_HAP_EN_GPIO		PM8921_GPIO_PM_TO_SYS(33)
//rupy DEL #define ISA1200_HAP_LEN_GPIO		PM8921_GPIO_PM_TO_SYS(20)
//rupy DEL #define ISA1200_HAP_CLK			PM8921_GPIO_PM_TO_SYS(44)

#define DRV2604_HAP_EN_GPIO		PM8921_GPIO_PM_TO_SYS(20) //rupy ADD
#define DRV2604_HAP_INTTRIG_GPIO		PM8921_GPIO_PM_TO_SYS(33) //rupy ADD
...
//rupy ADD START
static struct drv2604_regulator drv2604_reg_data[] = {
	{
		.name = "vddp",
		.min_uV = DRV_I2C_VTG_MIN_UV,
		.max_uV = DRV_I2C_VTG_MAX_UV,
		.load_uA = DRV_I2C_CURR_UA,
	},
};
//rupy ADD END
...
//rupy ADD START
static struct drv2604_platform_data drv2604_1_pdata = {
	.name = "vibrator",
	.hap_en_gpio = DRV2604_HAP_EN_GPIO,
	.hap_inttrig_gpio = DRV2604_HAP_INTTRIG_GPIO,
	.max_timeout = 15000,
	.regulator_info = drv2604_reg_data,
	.num_regulators = ARRAY_SIZE(drv2604_reg_data),
};
//rupy ADD END
#if 0 //rupy DEL START
...
#endif //rupy DEL END
//rupy ADD START
static struct i2c_board_info drv2604_board_info[] __initdata = {
	{
		I2C_BOARD_INFO("drv2604_1", 0x5A),
		.platform_data = &drv2604_1_pdata,
	},
};
//rupy ADD END
#if 0 /*<rupy>*/
...
/* Synaptics S7020 Touchpanel */
#define SYNAPTICS_INT_GPIO (32)
static struct regulator *tp_reg = NULL;

static void synaptics_dev_init(void)
{
	/* Interrupt */
	if (gpio_request(SYNAPTICS_INT_GPIO, "touch-int") < 0) {
		printk(KERN_ERR "can't get synaptics touch int down GPIO\n");
		return;
	}
	gpio_direction_input(SYNAPTICS_INT_GPIO);

	tp_reg = regulator_get(NULL, "tp_vdd");
	if (IS_ERR_OR_NULL(tp_reg)) {
		printk(KERN_ERR "touchpanel: regulator_get() failed\n");
		tp_reg = NULL;
	} else {
		int ret = 0;
		ret = regulator_set_voltage(tp_reg, 2850000, 2850000);
		if (ret != 0) {
			printk(KERN_ERR "touchpanel: regulator_set_voltage() failed, ret=%d\n", ret);
		}
		ret = regulator_set_optimum_mode(tp_reg, 150000);
		if (ret < 0) {
			printk(KERN_ERR "touchpanel: set_optimum_mode() failed, ret = %d\n", ret);
		}
		/* power on */
		regulator_enable(tp_reg);
	}
...
static int synaptics_power(int power_state)
{
	...
	switch (power_state) {
	case SYNAPTICS_POWER_OFF:
		if (tp_reg) {regulator_disable(tp_reg);}
		msleep(10); /* Wait for stablizing */
		break;
	case SYNAPTICS_POWER_ON:
		if (tp_reg) {regulator_enable(tp_reg);}
		msleep(400); /* Wait for stablizing */
		break;
	case SYNAPTICS_POWER_SET_PS:
		break;
	case SYNAPTICS_POWER_SET_NM:
		break;
	case SYNAPTICS_POWER_DEV_UP:
		synaptics_dev_init();
		if (tp_reg) {regulator_enable(tp_reg);}
		msleep(400); /* Wait for stablizing */
		break;
	case SYNAPTICS_POWER_DEV_DOWN:
		if (tp_reg) {regulator_disable(tp_reg);}
		msleep(10); /* Wait for stablizing */
		gpio_free(SYNAPTICS_INT_GPIO);
		break;
	case SYNAPTICS_POWER_RESTART:
		kernel_restart(NULL); /* restart system */
		break;
	default:
		break;
	}
...
static int synaptics_attn(void)
{
	int ret = ((gpio_get_value(SYNAPTICS_INT_GPIO) == 0) ? 0 : 1);
	return ret;
}
...
static struct i2c_board_info synaptics_s7020_info[] __initdata = {
	{
		I2C_BOARD_INFO(SYNAPTICS_S7020_NAME, 0x20),
		.irq		= MSM_GPIO_TO_INT(SYNAPTICS_INT_GPIO),
		.platform_data	= &synaptics_platform_data,
	},
};

#ifdef CONFIG_LEDS_PM8960
static struct i2c_board_info led_pm8960_info[] __initdata = {
	{
		I2C_BOARD_INFO("pm8xxx-i2c", 0x38),
	},
};
#endif /*CONFIG_LEDS_PM8960*/

/* Quad Fuelgauge IC */
static struct i2c_board_info safety_fuelgauge_info[] __initdata = {
	{
		I2C_BOARD_INFO("fuelgauge", 0x55),
	},
	{
		I2C_BOARD_INFO("fuelgauge_rom", 0x0B),
	},
	{
		I2C_BOARD_INFO("Spider3", 0x08),
	},
};
/* Quad Fuelgauge IC */

/* rupy-add-S */
static struct i2c_board_info felica_info[] __initdata = {
    {
        I2C_BOARD_INFO("felica_cen", 0x57),
    },
    {
        I2C_BOARD_INFO("subpmic", 0x6E),
    },
};
/* rupy-add-E */
...
/*	&apq8064_device_qup_i2c_gsbi2,*//*<rupy>*//* <Quad> DEL */
	&apq8064_device_qup_i2c_gsbi3,
	&apq8064_device_qup_i2c_gsbi4,
	&apq8064_device_qup_i2c_gsbi7,/*<rupy>*/
};
...
//	&apq8064_device_ext_ts_sw_vreg, /* Quad DEL */
	...
	&apq8064_device_tsif[1],
#else
	&apq8064_device_tsif[0],
#endif
#endif
};
...
static struct platform_device *sim_devices[] __initdata = {
	...
	&apq8064_device_uart_gsbi4,/*<rupy>*/
	&apq8064_device_uart_gsbi7,/*<rupy>*/
	&msm_device_sps_apq8064,
};

static struct platform_device *rumi3_devices[] __initdata = {
/*<rupy>	&apq8064_device_uart_gsbi1,*/
	&apq8064_device_uart_gsbi3,/*<rupy>*/
	&apq8064_device_uart_gsbi4,/*<rupy>*/
	&apq8064_device_uart_gsbi7,/*<rupy>*/
	&msm_device_sps_apq8064,
...
static struct msm_spi_platform_data apq8064_qup_spi_gsbi5_pdata = {
	.max_clock_speed = 48000000,	/* 48MHz camera_rupy */
};
...
/*<rupy>	.clk_freq = 100000,*/
	.clk_freq = 380000,/*<rupy>*/
	.src_clk_rate = 24000000,
};
/* <Quad> DEL Start */
#if 0
static struct msm_i2c_platform_data apq8064_i2c_qup_gsbi2_pdata = {/*<rupy>*/
	.clk_freq = 380000,
	.src_clk_rate = 24000000,
};
#endif
...
#endif
static struct msm_i2c_platform_data apq8064_i2c_qup_gsbi7_pdata = {/*<rupy>*/
	.clk_freq = 380000,
	.src_clk_rate = 24000000,
};
...
/* <Quad> DEL Start */
	...
	apq8064_device_qup_i2c_gsbi2.dev.platform_data =
					&apq8064_i2c_qup_gsbi2_pdata;/*<rupy>*/
#endif
/* <Quad> DEL End */
	apq8064_device_qup_i2c_gsbi3.dev.platform_data =
					&apq8064_i2c_qup_gsbi3_pdata;

  pr_debug("##Quad## %s: GSBI3 freq:%d rate:%d \n",
    __func__, apq8064_i2c_qup_gsbi3_pdata.clk_freq, apq8064_i2c_qup_gsbi3_pdata.src_clk_rate); /* Quad Debug */

	gsbi_mem = ioremap_nocache(MSM_GSBI3_PHYS, 4);/*<rupy>*/
	writel_relaxed(GSBI_DUAL_MODE_CODE, gsbi_mem);/*<rupy>*/
	/* Ensure protocol code is written before proceeding */
	wmb();/*<rupy>*/
	iounmap(gsbi_mem);/*<rupy>*/
	apq8064_i2c_qup_gsbi3_pdata.use_gsbi_shared_mode = 1;/*<rupy>*/

	apq8064_device_qup_i2c_gsbi1.dev.platform_data =
					&apq8064_i2c_qup_gsbi1_pdata;

  pr_debug("##Quad## %s: GSBI1 freq:%d rate:%d \n",
    __func__, apq8064_i2c_qup_gsbi1_pdata.clk_freq, apq8064_i2c_qup_gsbi1_pdata.src_clk_rate); /* Quad Debug */

	apq8064_device_qup_i2c_gsbi4.dev.platform_data =
					&apq8064_i2c_qup_gsbi4_pdata;

  pr_debug("##Quad## %s: GSBI4 freq:%d rate:%d \n",
    __func__, apq8064_i2c_qup_gsbi4_pdata.clk_freq, apq8064_i2c_qup_gsbi4_pdata.src_clk_rate); /* Quad Debug */

	gsbi_mem = ioremap_nocache(MSM_GSBI4_PHYS, 4);/*<rupy>*/
	writel_relaxed(GSBI_DUAL_MODE_CODE, gsbi_mem);/*<rupy>*/
	/* Ensure protocol code is written before proceeding */
	wmb();/*<rupy>*/
	iounmap(gsbi_mem);/*<rupy>*/
	apq8064_i2c_qup_gsbi4_pdata.use_gsbi_shared_mode = 1;/*<rupy>*/
#if 0	/* camera_rupy_2 */
	mpq8064_device_qup_i2c_gsbi5.dev.platform_data =
					&mpq8064_i2c_qup_gsbi5_pdata;
#endif
	apq8064_device_qup_i2c_gsbi7.dev.platform_data =
					&apq8064_i2c_qup_gsbi7_pdata;/*<rupy>*/

  pr_debug("##Quad## %s: GSBI7 freq:%d rate:%d \n",
    __func__, apq8064_i2c_qup_gsbi7_pdata.clk_freq, apq8064_i2c_qup_gsbi7_pdata.src_clk_rate); /* Quad Debug */

	gsbi_mem = ioremap_nocache(MSM_GSBI7_PHYS, 4);/*<rupy>*/
	writel_relaxed(GSBI_DUAL_MODE_CODE, gsbi_mem);/*<rupy>*/
	/* Ensure protocol code is written before proceeding */
	wmb();/*<rupy>*/
	iounmap(gsbi_mem);/*<rupy>*/
	apq8064_i2c_qup_gsbi7_pdata.use_gsbi_shared_mode = 1;/*<rupy>*/
}
...
/* Quad ADD START 12/07/24 */
#define GPIO_KEY_BACK		PM8921_GPIO_PM_TO_SYS(15)
#define GPIO_KEY_MENU		PM8921_GPIO_PM_TO_SYS(9)
/* Quad ADD END   12/07/24*/
/*<rupy>#define GPIO_KEY_ROTATION      PM8921_GPIO_PM_TO_SYS(42)*/
...
//rupy ADD START
	{
		...
		APQ_8064_GSBI1_QUP_I2C_BUS_ID,
		drv2604_board_info,
		ARRAY_SIZE(drv2604_board_info),
	},
//rupy ADD END
	{
		I2C_SURF | I2C_FFA,
		APQ_8064_GSBI3_QUP_I2C_BUS_ID,
		synaptics_s7020_info,
		ARRAY_SIZE(synaptics_s7020_info),
	},
#ifdef CONFIG_LEDS_PM8960
	{
		I2C_SURF | I2C_FFA | I2C_LIQUID,
...
/* Quad Fuelgauge IC */
	{
		...
		safety_fuelgauge_info,
		ARRAY_SIZE(safety_fuelgauge_info),
	},
/* Quad Fuelgauge IC */
...
static void __init register_i2c_devices(void)
{
	...
	printk(KERN_ERR "#######  board-8064: mach_mask = %0x ##########\n", mach_mask);
...
/* IDP AlarmWatch: Enable last_kmsg */
#ifdef CONFIG_ANDROID_RAM_CONSOLE
#ifndef DMEM_RAM_CONSOLE_SIZE
#define DMEM_RAM_CONSOLE_SIZE   (0x20000)
#endif
#ifndef DMEM_RAM_CONSOLE_START
/*#define DMEM_RAM_CONSOLE_START  (0xBFF40000)*//* iDPower memory map */
#define DMEM_RAM_CONSOLE_START    (0x90140000)  /* Rupy memory map */
#endif
static struct resource ram_console_resource __devinitdata = {
       .start  = DMEM_RAM_CONSOLE_START,
       .end    = (DMEM_RAM_CONSOLE_START + DMEM_RAM_CONSOLE_SIZE - 1),
       .flags  = IORESOURCE_MEM,
};


static struct platform_device ram_console_device __devinitdata = {
       .name = "ram_console",
       .id = 0,
       .num_resources  = 1,

...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
DEFINE_MUTEX(mipi_sharp_vsync_lock);
static unsigned int sharp_cmd_vsync_flg = 0;
#define D_TMD_MDP_USE_OFF 0x00
#define D_TMD_MDP_VS_USE 0x01
#define D_TMD_CMD_VS_USE 0x02
#define D_TMD_MDP_OTHER_USE 0x03
static struct completion dsi_vblank_event;
struct timer_list mipi_vsync_failsafe_timer;
#endif
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* npdc300060183 */
	if(intr == INTR_PRIMARY_VSYNC)
		sharp_cmd_vsync_flg |= D_TMD_MDP_VS_USE;
#endif	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* npdc300060183 */
	mdp_enable_irq(term);
...
static void vsync_irq_disable(int intr, int term)
{
	...
	spin_lock_irqsave(&mdp_spin_lock, flag);
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* npdc300060183 */
	if(!((intr == INTR_PRIMARY_VSYNC) && (sharp_cmd_vsync_flg & ~D_TMD_MDP_VS_USE))) {
#endif	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* npdc300060183 */
	outp32(MDP_INTR_CLEAR, intr);
	mdp_intr_mask &= ~intr;
	outp32(MDP_INTR_ENABLE, mdp_intr_mask);
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD /*Sequence*/
	vsync_irq_enable(INTR_PRIMARY_VSYNC, MDP_PRIM_VSYNC_TERM);
#endif
	/* start timing generator & mmu if they are not started yet */
	mdp4_overlay_dsi_video_start();
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
static void mipi_vsync_failsafe_handler(unsigned long data)
{	
	complete_all(&dsi_vblank_event);
}
#endif
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
		init_timer(&mipi_vsync_failsafe_timer);
		mipi_vsync_failsafe_timer.function = mipi_vsync_failsafe_handler;
		mipi_vsync_failsafe_timer.data = 0;
		init_completion(&dsi_vblank_event);
#endif
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
	mdp4_overlay_rgb_blackscreen_setup(pipe);
	mdp4_overlayproc_cfg(pipe);
	
	mdp4_overlay_reg_flush(pipe, 1);
	mdp4_mixer_stage_gfx_up(pipe);
	mdp4_mixer_stage_gfx_commit(pipe->mixer_num);
#else
...
int mdp4_dsi_video_off(struct platform_device *pdev)
{
	...
	if (pipe->ov_blt_addr) {
		...
		mdp4_overlay_mdp_perf_current_set_use_ov0_blt( 0 );
#endif  /* CONFIG_FB_MSM_MIPI_DSI_TMD */
	}

	mdp_histogram_ctrl_all(FALSE);
#ifdef MIPI_SHARP_DEBUG_LOG_ALL
	printk("[%s]######### LCD Sequence 3-5 #########\n",__func__);
#endif /*MIPI_SHARP_DEBUG_LOG_ALL*/
	MDP_OUTP(MDP_BASE + DSI_VIDEO_BASE, 0);
...
#ifdef MIPI_SHARP_DEBUG_LOG
{
	...
	dsi_ctrl = MIPI_INP(MDP_BASE + DSI_VIDEO_BASE);
	video_mode = dsi_ctrl & 0x01; /* DSI_VIDEO_EN */
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD /*Sequence*/
	mipi_sharp_lcd_off_cmd(pdev);
#endif

#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
	msleep(20); //npdc300029273
#endif
	...
	if (pipe) {
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD	/* npdc300053106 */
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
void mdp4_overlay_dsi_video_wait4vblank(struct msm_fb_data_type *mfd)
{
	unsigned long flag;
	unsigned int data;
	struct vsycn_ctrl *vctrl;
	...
	mutex_lock(&mipi_sharp_vsync_lock);
	spin_lock_irqsave(&mdp_spin_lock, flag);
	...
	sharp_cmd_vsync_flg |= D_TMD_CMD_VS_USE;
	if(!sharp_cmd_vsync_flg)
	{
		if (vctrl->vsync_irq_enabled == 0){
			outp32(MDP_INTR_CLEAR, INTR_PRIMARY_VSYNC); 
			mdp_intr_mask |= INTR_PRIMARY_VSYNC; 
			outp32(MDP_INTR_ENABLE, mdp_intr_mask);
		}
		spin_unlock_irqrestore(&mdp_spin_lock, flag);
		mdp_enable_irq(MDP_VBLANK_TERM);
	}
	else
	{
		spin_unlock_irqrestore(&mdp_spin_lock, flag);
	...
	mipi_vsync_failsafe_timer.expires = jiffies + msecs_to_jiffies( 500 );

	add_timer(&mipi_vsync_failsafe_timer); //failsafe timer

	wait_for_completion(&dsi_vblank_event);
	
	del_timer(&mipi_vsync_failsafe_timer);
	...
	spin_unlock_irqrestore(&mdp_spin_lock, flag);
	mdp_disable_irq(MDP_VBLANK_TERM);
	mutex_unlock(&mipi_sharp_vsync_lock);
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
void mdp4_primary_vsync_dsi_cmd_sync(void)
{
	complete_all(&dsi_vblank_event);
}
#endif
...
void mdp4_dsi_video_overlay(struct msm_fb_data_type *mfd)
{
	...
	struct vsycn_ctrl *vctrl;
	struct mdp4_overlay_pipe *pipe;
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD		/* npdc300085539 */
	struct mdp4_overlay_pipe *bg_pipe, *fg_pipe, *pp, *real_pipe;
	int i, undx;
	struct vsync_update *vp;
	bool is_external = false;
#endif	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* npdc300085539 */
	...
	for (i = 0; i < OVERLAY_PIPE_MAX; i++) {
        ...
        if( real_pipe && real_pipe->mixer_num == MDP4_MIXER2){
	        is_external = true;
		}
	}
	...
	if( is_external == true ){
		bg_pipe = pipe;
		for (i = 1; i < OVERLAY_PIPE_MAX; i++) {	/* ndx start from 1 */
			fg_pipe = mdp4_overlay_ndx2pipe(i);
			if ((fg_pipe && fg_pipe->pipe_used) &&
			    (fg_pipe->pipe_num != bg_pipe->pipe_num) &&
			    (fg_pipe->mixer_num == bg_pipe->mixer_num) &&
			    (fg_pipe->is_fg)) {
				undx =  vctrl->update_ndx;
				vp = &vctrl->vlist[undx];
				pp = &vp->plist[fg_pipe->pipe_ndx -1];
				if (!(pp->pipe_used)) {
					pr_debug("MDP_debug[%s] foreground pipe_num=%d mixer_num=%d "
						"mixer_stage=%d didn't play!\n", __func__, fg_pipe->pipe_num,
						fg_pipe->mixer_num, fg_pipe->mixer_stage);
	...
	if ((cnt != 0) && (pipe->ov_blt_addr == 0) && (vctrl->blt_free == 0)){
		/* Calculation of the bus bw total value of each layer */
		mdp4_overlay_mdp_perf_req_down(mfd);
	}
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
void mdp4_dsi_video_blackscreen_overlay(struct msm_fb_data_type *mfd)
{
	struct fb_info *fbi = mfd->fbi;
	uint8 *buf;
	int bpp;
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD			/* BUS_BW_CUSTOM */
struct msm_fb_data_type * mdp4_overlay_dsi_video_get_mfd(void)
{
	return vsync_ctrl_db[0].mfd;
}
#endif	/* CONFIG_FB_MSM_MIPI_DSI_TMD */	/* BUS_BW_CUSTOM */

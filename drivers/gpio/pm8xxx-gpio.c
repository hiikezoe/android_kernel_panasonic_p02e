...
static struct pm_gpio_chip *the_pm_gpio_chip; /* npdc300020773 */
...
static int __devinit pm_gpio_probe(struct platform_device *pdev)
{
	...
	the_pm_gpio_chip = pm_gpio_chip; /* npdc300020773 */
...
static int __devexit pm_gpio_remove(struct platform_device *pdev)
{
	...
	mutex_unlock(&pm_gpio_chips_lock);
	platform_set_drvdata(pdev, NULL);
	the_pm_gpio_chip = NULL; /* npdc300020773 */
	if (gpiochip_remove(&pm_gpio_chip->gpio_chip))
		pr_err("failed to remove gpio chip\n");
	kfree(pm_gpio_chip->bank1);
	kfree(pm_gpio_chip);
...
/* npdc300020773 start */
int pm8xxx_gpio_read(unsigned gpio)
{
	return pm_gpio_get(the_pm_gpio_chip, gpio);
}
EXPORT_SYMBOL(pm8xxx_gpio_read);
/* npdc300020773 end */

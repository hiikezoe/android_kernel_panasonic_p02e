...
#define WFS_PAUSE_WAIT    500 //msec
#define WFS_FORMAT_SIZE   3/2
#define WFS_PAUSE_DATA_SIZEMAX 0x152000
#define WFD_PAUSE_DATA_ALIGN   0x800

static void * wfd_pausedata_vart_adr;

static void * wfd_escapedata_vart_adr;            /* PMC-Quad-WFD-121214 */

static struct kobject *wfd_kobj;
static void   *force_pause_inst = NULL;
static int    wfd_force_pause   = 0;
static int    wfd_pause_mode    = 0;
static int    wfd_pause_mode_bk = 0;
static int    wfd_pause_width   = 0;
static int    wfd_pause_height  = 0;
static u16    wfd_pause_string_width = 0;
static u16    wfd_pause_string_height = 0;
static u8     wfd_pause_string_copy  = 0;
static u32    wfd_pause_string_count = 0;
static u32    wfd_pause_string_size  = 0;
static u8    *wfd_pause_string_data = NULL;
extern void mdp_force_pause(void *);
struct wfd_device {
...
/* PMC-Quad-WFD-121214 Start */
static int wfd_clean_ion_caches(struct ion_client *client,
		struct mem_region *mregion)
{
	if (!client || !mregion) {
		WFD_MSG_ERR("Failed to clean caches: "
				"Invalid client or region");
		return -EINVAL;
	} else if (!mregion->ion_handle) {
		WFD_MSG_ERR("Failed to clean caches: "
	...
	while (!kthread_should_stop()) {
		...
		if (!mregion) {
			...
			continue;
                ...
                if( wfd_pause_mode == 1 )
                {
                        memcpy( wfd_escapedata_vart_adr, mregion->kvaddr, mregion->size );/* PMC-Quad-WFD-121214 */
                        memcpy( mregion->kvaddr, wfd_pausedata_vart_adr, mregion->size );
                        wfd_clean_ion_caches(wfd_dev->ion_client, mregion);               /* PMC-Quad-WFD-121214 */
                }
		...
		struct v4l2_buffer *b)
{
	...
	if (rc)
		...
		wfd_stats_update(&inst->stats, WFD_STAT_EVENT_CLIENT_QUEUE);
                force_pause_inst = inst->mdp_inst;
	}
	return rc;
}
...
static int vsg_encode_frame(void *cookie, struct vsg_buf_info *buf)
{
	...
	venc_buf = (struct venc_buf_info){
        ...
        if( wfd_pause_mode != wfd_pause_mode_bk )
        {
                if( wfd_pause_mode == 1 )
                {
/* PMC-Quad-WFD-121214 Start */
                        memcpy( wfd_escapedata_vart_adr,
                                ( venc_buf.mregion )->kvaddr,
                                ( venc_buf.mregion )->size );
/* PMC-Quad-WFD-121214 End */
                        memcpy(( venc_buf.mregion )->kvaddr,
                               wfd_pausedata_vart_adr,
                               ( venc_buf.mregion )->size );
                }
/* PMC-Quad-WFD-121214 Start */
                else
                {
                        memcpy(( venc_buf.mregion )->kvaddr,
                               wfd_escapedata_vart_adr,
                               ( venc_buf.mregion )->size );
                }
/* PMC-Quad-WFD-121214 End */
                wfd_pause_mode_bk = wfd_pause_mode;
/* PMC-Quad-WFD-121214 Start */
                wfd_clean_ion_caches(wfd_dev->ion_client, venc_buf.mregion);
/* PMC-Quad-WFD-121214 End */
        }
...
static int wfd_pause_screen_create( void )
{
        u8 *y_data_adr;
        u8 *uv_data_adr;
        u8 *y_sdata_adr;
        ...
        u32 cnt1;
        u32 cnt2;
        u32 y_vertical_offset;
        u32 y_horizontal_offset;
        u32 uv_vertical_offset;
        u32 uv_horizontal_offset;



        if(( wfd_pausedata_vart_adr == NULL )||( wfd_pause_string_data == NULL ))
        {
		WFD_MSG_ERR("Input buffer address error\n");
                return -1;
        }

        if(( wfd_pause_height <= wfd_pause_string_height )
        || ( wfd_pause_width  <= wfd_pause_string_width )
        || ( wfd_pause_height >  720 )
        || ( wfd_pause_width  >  1280 )
        || (( wfd_pause_string_height % 2 )!= 0 )
        || (( wfd_pause_string_width  % 2 )!= 0 )
        || (( wfd_pause_height % 2 )!= 0 )
        || (( wfd_pause_width  % 2 )!= 0 ))
        {
		WFD_MSG_ERR("Input size error\n");
                return -1;
        }

        y_data_adr = (u8 *)wfd_pausedata_vart_adr;
        align = ( wfd_pause_width * wfd_pause_height )% WFD_PAUSE_DATA_ALIGN;
        if( align != 0 )
        {
                align = WFD_PAUSE_DATA_ALIGN - align;
        }
        uv_data_adr = y_data_adr + ( wfd_pause_width * wfd_pause_height ) + align;

        y_sdata_adr = wfd_pause_string_data;
        u_sdata_adr = y_sdata_adr + ( wfd_pause_string_width * wfd_pause_string_height );
        v_sdata_adr = u_sdata_adr + (( wfd_pause_string_width * wfd_pause_string_height )/4);

        //set black color
        memset( y_data_adr, 0x00, (wfd_pause_width*wfd_pause_height));
        memset( uv_data_adr, 0x80, (wfd_pause_width*wfd_pause_height)/2);

        //y_data
        y_vertical_offset = ( wfd_pause_height - wfd_pause_string_height )/2;
        y_horizontal_offset = ( wfd_pause_width - wfd_pause_string_width )/2;

        y_data_filladr = y_data_adr + ( wfd_pause_width * y_vertical_offset ) + y_horizontal_offset;
        y_sdata_filladr = y_sdata_adr;

        for( cnt1 = 0; cnt1 < wfd_pause_string_height; cnt1++ )
        {
                for( cnt2 = 0; cnt2 < wfd_pause_string_width; cnt2++ )
                {
                        *( y_data_filladr + cnt2 ) = *( y_sdata_filladr + cnt2 );
                }
                y_data_filladr = y_data_filladr + wfd_pause_width;
                y_sdata_filladr = y_sdata_filladr + wfd_pause_string_width;
        }

        //u_data, v_data
        uv_vertical_offset = y_vertical_offset/2;
        uv_horizontal_offset = y_horizontal_offset/2;

        uv_data_filladr = uv_data_adr + ((( wfd_pause_width/2 )* uv_vertical_offset )* 2 ) + ( uv_horizontal_offset * 2 );
        u_sdata_filladr = u_sdata_adr;
        v_sdata_filladr = v_sdata_adr;

        for( cnt1 = 0; cnt1 < ( wfd_pause_string_height/2 ); cnt1++ )
        {
                for( cnt2 = 0; cnt2 < ( wfd_pause_string_width/2 ); cnt2++ )
                {
                        *( uv_data_filladr + ( cnt2 * 2 )) = *( u_sdata_filladr + cnt2 );
                        *( uv_data_filladr + ( cnt2 * 2 ) + 1 ) = *( v_sdata_filladr + cnt2 );
                }
                uv_data_filladr = uv_data_filladr + (( wfd_pause_width/2 )* 2 );
                u_sdata_filladr = u_sdata_filladr + ( wfd_pause_string_width/2 );
                v_sdata_filladr = v_sdata_filladr + ( wfd_pause_string_width/2 );
        }

        return 0;
}

static ssize_t sysfs_wfd_force_pause_show( struct kobject *kobj, struct kobj_attribute * attr, char *buf )
{
        //WFD_MSG_DBG("sysfs_wfd_force_pause_show--->wfd_force_pause:%d\n",wfd_force_pause);
        printk("sysfs_wfd_force_pause_show--->wfd_force_pause:%d\n",wfd_force_pause);
        return sprintf( buf, "%s:%d\n", attr->attr.name, wfd_force_pause );
}
static ssize_t sysfs_wfd_force_pause_store( struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count )
{
        sscanf( buf, "%du",&wfd_force_pause );
        //WFD_MSG_DBG("sysfs_wfd_force_pause_store--->wfd_force_store:%d bk:%d\n",wfd_force_pause,wfd_force_pause);
        printk("sysfs_wfd_force_pause_store--->wfd_force_store:%d bk:%d\n",wfd_force_pause,wfd_force_pause);

        if( wfd_force_pause == 1 )
        {
            if (force_pause_inst != NULL ) {
            printk("%s: call force_pause\n",__func__);
            mdp_force_pause(force_pause_inst);
            } else {
            printk("%s: cannot call force_pause because null\n",__func__);
            }
        }
...
static ssize_t sysfs_wfd_pause_mode_show( struct kobject *kobj, struct kobj_attribute * attr, char *buf )
{
        WFD_MSG_DBG("sysfs_wfd_pause_mode_show--->wfd_pause_mode:%d\n",wfd_pause_mode);
        return sprintf( buf, "%s:%d\n", attr->attr.name, wfd_pause_mode );
}
static ssize_t sysfs_wfd_pause_mode_store( struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count )
{
        sscanf( buf, "%du",&wfd_pause_mode );
        WFD_MSG_DBG("sysfs_wfd_pause_mode_store--->wfd_pause_mode:%d bk:%d\n",wfd_pause_mode,wfd_pause_mode);

        if( wfd_pause_mode == 1 )
        {
                set_current_state( TASK_INTERRUPTIBLE );
                schedule_timeout(( WFS_PAUSE_WAIT * HZ )/1000);
        }

        return count;
}
static ssize_t sysfs_wfd_pause_width_show( struct kobject *kobj, struct kobj_attribute * attr, char *buf )
{
        WFD_MSG_DBG("sysfs_wfd_pause_width_show--->wfd_pause_width:%d\n",wfd_pause_width);
        return sprintf( buf, "%s:%d\n", attr->attr.name, wfd_pause_width );
}
static ssize_t sysfs_wfd_pause_width_store( struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count )
{
        sscanf( buf, "%du", &wfd_pause_width );
        WFD_MSG_DBG("sysfs_wfd_pause_width_store--->wfd_pause_width:%d\n",wfd_pause_width);
        return count;
}
static ssize_t sysfs_wfd_pause_height_show( struct kobject *kobj, struct kobj_attribute * attr, char *buf )
{
        WFD_MSG_DBG("sysfs_wfd_pause_height_show--->wfd_pause_height:%d\n",wfd_pause_height);
        return sprintf( buf, "%s:%d\n", attr->attr.name, wfd_pause_height );
}
static ssize_t sysfs_wfd_pause_height_store( struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count )
{
        int ret;

        sscanf( buf, "%du", &wfd_pause_height );
        WFD_MSG_DBG("sysfs_wfd_pause_height_store--->wfd_pause_height:%d\n",wfd_pause_height);

        ret = wfd_pause_screen_create();
        if( ret < 0 )
        {
                WFD_MSG_ERR("sysfs_wfd_pause_height_store wfd_pause_screen_create Err %d\n",ret);
        }
        return count;
}
static ssize_t sysfs_wfd_pause_string_data_show( struct kobject *kobj, struct kobj_attribute * attr, char *buf )
{
        WFD_MSG_DBG("sysfs_wfd_pause_string_data_show--->wfd_pause_string_data:%d\n",( u32 )( wfd_pause_string_data ));
        return sprintf( buf, "%s:%d\n", attr->attr.name, ( u32 )wfd_pause_string_data );
}
static ssize_t sysfs_wfd_pause_string_data_store( struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count )
{
        if( wfd_pause_string_copy == 0 )
        {
                wfd_pause_string_width = *(( u16 * )buf);
                wfd_pause_string_height = *(( u16 * )(buf + 2));

                if( wfd_pause_string_data != NULL )
                {
                        kfree( wfd_pause_string_data );
                        wfd_pause_string_data = NULL;
                }

                wfd_pause_string_size = wfd_pause_string_width * wfd_pause_string_height * WFS_FORMAT_SIZE;

                if(( wfd_pause_string_width  <= DEFAULT_WFD_WIDTH )
                && ( wfd_pause_string_height <= DEFAULT_WFD_HEIGHT ))
                {
                        wfd_pause_string_data = kmalloc( wfd_pause_string_size, GFP_KERNEL );
                }
                else
                {
                        WFD_MSG_ERR("Failed string size ...\n");
                        wfd_pause_string_size = DEFAULT_WFD_WIDTH * DEFAULT_WFD_HEIGHT * WFS_FORMAT_SIZE;
                }


                if( wfd_pause_string_data != NULL )
                {
                        memcpy( wfd_pause_string_data, buf + 4, count - 4 );
                }
                else
                {
                        WFD_MSG_ERR("Failed kmalloc string data...\n");
                }

                wfd_pause_string_count += count - 4;
                wfd_pause_string_copy = 1;
        }
        else
        {
                if( wfd_pause_string_data != NULL )
                {
                        memcpy(( wfd_pause_string_data + wfd_pause_string_count ), buf, count );
                }
                else
                {
                        WFD_MSG_ERR("Failed kmalloc string data...\n");
                ...
                wfd_pause_string_count += count;

                if( wfd_pause_string_count >= wfd_pause_string_size )
                {
                        wfd_pause_string_count = 0;
                        wfd_pause_string_size  = 0;
                        wfd_pause_string_copy  = 0;
                }
        }

        WFD_MSG_DBG("sysfs_wfd_pause_string_data_store--->wfd_pause_string_data:%d\n",( u32 )wfd_pause_string_data);
        return count;
}
static struct kobj_attribute wfd_force_pause_attribute = __ATTR( wfd_force_pause,
                                                                0664,
                                                                sysfs_wfd_force_pause_show,
                                                                sysfs_wfd_force_pause_store );
static struct kobj_attribute wfd_pause_mode_attribute = __ATTR( wfd_pause_mode,
                                                                0664,
                                                                sysfs_wfd_pause_mode_show,
                                                                sysfs_wfd_pause_mode_store );
static struct kobj_attribute wfd_pause_width_attribute = __ATTR( wfd_pause_width,
                                                                 0664,
                                                                 sysfs_wfd_pause_width_show,
                                                                 sysfs_wfd_pause_width_store );
static struct kobj_attribute wfd_pause_height_attribute = __ATTR( wfd_pause_height,
                                                                  0664,
                                                                  sysfs_wfd_pause_height_show,
                                                                  sysfs_wfd_pause_height_store );
static struct kobj_attribute wfd_pause_string_data_attribute = __ATTR( wfd_pause_string_data,
                                                                       0664,
                                                                       sysfs_wfd_pause_string_data_show,
                                                                       sysfs_wfd_pause_string_data_store );
static struct attribute * attrs[] =
{
        &wfd_force_pause_attribute.attr,
        &wfd_pause_mode_attribute.attr,
        &wfd_pause_width_attribute.attr,
        &wfd_pause_height_attribute.attr,
        &wfd_pause_string_data_attribute.attr,
...
static int __init wfd_init(void)
{
	...
	if (rc) {
        ...
        wfd_kobj = kobject_create_and_add("wfd", kernel_kobj );
        if( wfd_kobj == NULL )
        {
                WFD_MSG_ERR("failed to kobject \n");
                platform_driver_unregister(&wfd_driver);
                goto err_platform_registration;
        }

        rc = sysfs_create_group( wfd_kobj, &attr_group );
        if( rc != 0 )
        {
                WFD_MSG_ERR("failed to sysfs_create_group \n");
                kobject_put( wfd_kobj );
                platform_driver_unregister(&wfd_driver);
                goto err_platform_registration;
        }

        wfd_pausedata_vart_adr = kmalloc( WFS_PAUSE_DATA_SIZEMAX, GFP_KERNEL );
        if( wfd_pausedata_vart_adr != NULL )
        {
                WFD_MSG_DBG("kmalloc area init ...\n");
                memset( (u8 *)wfd_pausedata_vart_adr, 0x00, WFS_PAUSE_DATA_SIZEMAX );
        }
        else
        {
                WFD_MSG_ERR("Failed kmalloc ...\n");
        }
/* PMC-Quad-WFD-121214 Start */
        wfd_escapedata_vart_adr = kmalloc( WFS_PAUSE_DATA_SIZEMAX, GFP_KERNEL );
        if( wfd_escapedata_vart_adr != NULL )
        {
                WFD_MSG_DBG("kmalloc area init ...\n");
                memset( (u8 *)wfd_escapedata_vart_adr, 0x00, WFS_PAUSE_DATA_SIZEMAX );
        }
        else
        {
                WFD_MSG_ERR("Failed kmalloc for escapedata...\n");
        }
/* PMC-Quad-WFD-121214 End */
...
/* PMC-Quad-WFD-121214 Start */
        kfree( wfd_escapedata_vart_adr );
        kfree( wfd_pausedata_vart_adr );
/* PMC-Quad-WFD-121214 End */
        kobject_put( wfd_kobj );
	platform_driver_unregister(&wfd_driver);
}

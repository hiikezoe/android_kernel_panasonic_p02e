...
struct timer_list dsi_dma_comp_failsafe_timer;
DEFINE_MUTEX(mipi_dsi_dma_comp_lock);
DEFINE_MUTEX(mipi_dsi_sharp_lock);
#endif
...
static void dsi_dma_comp_failsafe_handler(unsigned long data)
{
	complete_all(&dsi_dma_comp);
}
...
void mipi_dsi_init(void)
{
	...
	init_timer(&dsi_dma_comp_failsafe_timer);
	dsi_dma_comp_failsafe_timer.function = dsi_dma_comp_failsafe_handler;
	dsi_dma_comp_failsafe_timer.data = 0;
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
void mipi_dsi_sharp_cmd_sync(struct msm_fb_data_type *mfd)
{
	int video_mode;
	uint32 dsi_ctrl;
	...
	if (video_mode)
	{
		mdp4_overlay_dsi_video_wait4vblank(mfd);
	}
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
	mutex_lock(&mipi_dsi_sharp_lock);
#endif
	/* turn on cmd mode
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
	mutex_unlock(&mipi_dsi_sharp_lock);
#endif
...
int mipi_dsi_cmd_dma_tx(struct dsi_buf *tp)
{
	...
	dsi_dma_comp_failsafe_timer.expires = jiffies + msecs_to_jiffies( 30 );

	mutex_lock(&mipi_dsi_dma_comp_lock);

	add_timer(&dsi_dma_comp_failsafe_timer);

	wait_for_completion(&dsi_dma_comp);

	del_timer(&dsi_dma_comp_failsafe_timer);

	mutex_unlock(&mipi_dsi_dma_comp_lock);

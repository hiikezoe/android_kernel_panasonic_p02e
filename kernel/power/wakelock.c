...
volatile atomic_t debug_suspend_status = ATOMIC_INIT(0);	/* debug for resume irq */

/* QUAD npdc300050358 start */
#include <linux/irq.h>
#define WAKEUP_IRQ_LOG_NUM 30

struct workqueue_struct* wakeup_irq_debug_work_queue;
static unsigned int irq_log_count = 0;
static bool is_irq_logging = false;
static unsigned int wakeup_irq[WAKEUP_IRQ_LOG_NUM];

void notify_wakeup_irq(unsigned int irq)
{
	if((is_irq_logging)
	&& (irq_log_count < WAKEUP_IRQ_LOG_NUM)){
		wakeup_irq[irq_log_count++] = irq;
	}
}

void start_irq_log(void)
{
	is_irq_logging = true;
}

static void wakeup_irq_debug(struct work_struct *work)
{
	struct timespec ts;
	struct rtc_time tm;
	int i;
	...
	if(irq_log_count >= WAKEUP_IRQ_LOG_NUM){
		pr_info("interrupt count exceeds %d\n", irq_log_count);
	}
	else{
	...
	for(i = 0; i < irq_log_count; i++){
		...
		desc = irq_to_desc(wakeup_irq[i]);
		pr_info("interrupt(%d): %s triggered (%d-%02d-%02d %02d:%02d:%02d.%09lu UTC)\n",
			wakeup_irq[i], desc->action->name, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
			tm.tm_hour, tm.tm_min, tm.tm_sec, ts.tv_nsec);
	}
	irq_log_count = 0;
}
static DECLARE_WORK(wakeup_irq_debug_work, wakeup_irq_debug);

static void flush_irq_log(void)
{
	is_irq_logging = false;
	queue_work(wakeup_irq_debug_work_queue, &wakeup_irq_debug_work);
}
/* QUAD npdc300050358 end */
...
static void suspend(struct work_struct *work)
{
	...
	atomic_set(&debug_suspend_status, 1);	/* debug for resume irq */
	getnstimeofday(&ts_entry);
	ret = pm_suspend(requested_suspend_state);
	getnstimeofday(&ts_exit);
	atomic_set(&debug_suspend_status, 0);	/* debug for resume irq */
	...
	struct wake_lock *lock, long timeout, int has_timeout)
{
	...
	if (type == WAKE_LOCK_SUSPEND && wait_for_wakeup) {
		flush_irq_log();    /* QUAD npdc300050358 */
...
static int __init wakelocks_init(void)
{
	...
	/* QUAD npdc300050358 start */
	wakeup_irq_debug_work_queue = create_singlethread_workqueue("wakeup_irq_debug");
	if (wakeup_irq_debug_work_queue == NULL) {
		ret = -ENOMEM;
		goto err_wakeup_irq_debug_work_queue;
	}
	/* QUAD npdc300050358 end */
...
err_wakeup_irq_debug_work_queue:    /* QUAD npdc300050358 */
...
#endif
	destroy_workqueue(wakeup_irq_debug_work_queue); /* QUAD npdc300050358 */

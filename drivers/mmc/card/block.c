...
//	if ((!capable(CAP_SYS_RAWIO)) || (bdev != bdev->bd_contains))
	...
	if (idata->buf_bytes) {
		...
		data.timeout_ns = data.timeout_ns * 2;
...
//merge s
	else if (cmd == MMC_EXT_CMD) {
		struct mmc_ext_cmd __user *usrExtCmd = (struct mmc_ext_cmd __user *)arg;
		struct mmc_ext_cmd extCmd;
		struct mmc_blk_data *md;
		struct mmc_card *card;
		
		if (copy_from_user(&extCmd, usrExtCmd, sizeof(struct mmc_ext_cmd))) {
			return -EFAULT;
		}
		...
		switch (extCmd.opcode) {
		case MMC_EXT_GET_RCA:
			if (copy_to_user((void __user *)(unsigned int*)extCmd.resp, &card->rca, sizeof(unsigned int))) {
				ret = -EFAULT;
			} else {
				ret = 0;
			}
			break;
		case MMC_EXT_INIT_CARD:
			if (card->host->bus_ops->resume != NULL) {
				ret = card->host->bus_ops->resume(card->host);
			} else {
				ret = -EINVAL;
			}
			break;
	...
	struct mmc_blk_request *brq, int *ecc_err)
{
	...
	printk(KERN_ERR "[MMC] %s: error %d at mmc_blk_cmd_recovery(), sector %u, nr %u, cmd response %#x, card status %#x\n",
				req->rq_disk->disk_name, brq->data.error,
				(unsigned)blk_rq_pos(req),
				(unsigned)blk_rq_sectors(req),
				brq->cmd.resp[0], brq->stop.resp[0]);
	printk(KERN_ERR "[MMC] rq_data_dir(req) = %d\n", rq_data_dir(req) ); // READ=0;WRITE=1
	printk(KERN_ERR "[MMC] MID=%X OEMID=%X P_NAME=%c%c%c%c%c \n",
...
/* CHG-S [SDDriver_005] */
				if ( retry_cmd13_chk < 800 )
				{
					retry_cmd13_chk++;
				}
				else
				{
					printk(KERN_ERR "[MMC] On Write process wait for R1_READY_FOR_DATA NG, card is Busy Freeze \n");
					return MMC_BLK_ABORT;
				}
/* CHG-E [SDDriver_005] */
			       ...
			       struct mmc_queue *mq)
{
	...
	brq->data.timeout_ns = brq->data.timeout_ns * 2;
...
static int mmc_blk_issue_rw_rq(struct mmc_queue *mq, struct request *rqc)
{
	...
	u8 reqs = 0;
/* ADD-S [SDDriver_003][SDDriver_004] */
	int retry_chk = 0;
/* ADD-E [SDDriver_003][SDDriver_004] */
	int init_err = 0;
	int init_retries;
	...
	if( ( card->type == MMC_TYPE_SD ) &&
		( ( card->cid.manfid == 0x27 ) || ( card->cid.manfid == 0x13 ) ) )
	{
		card->host->max_blk_count = 64;
	}
...
/* ADD-S [SDDriver_004] */
			if ( retry_chk < 10 )
			{
				...
				if ( retry_chk > 4 )
				{
					printk(KERN_ERR "[MMC] SD read retry : mmc_power_off() call : retry_cnt = %d\n", retry_chk);
					mmc_power_off( card->host );
					printk(KERN_ERR "[MMC] SD read retry : mmc_power_off() called\n");
					mmc_delay(100);
					mmc_power_up( card->host );
					printk(KERN_ERR "[MMC] SD read retry : mmc_power_up() called\n");
					mmc_select_voltage( card->host ,  card->host->ocr);
					init_retries = 5;
					while(init_retries) {
						printk(KERN_ERR "[MMC] SD read retry : mmc_sd_init_card() call : init_retries = %d\n", init_retries);
						init_err = mmc_sd_init_card(card->host, card->host->ocr, card->host->card);
						if( init_err )
						{
							printk(KERN_ERR "[MMC] Re-mmc_init_sd_card \n");
							init_retries--;
							mmc_power_off(card->host);
							mmc_delay(100);
							mmc_power_up(card->host);
					...
					if (!init_retries) {
						printk(KERN_ERR "[MMC] Re-mmc_init_sd_card ERR\n");
						goto cmd_abort;
					}
				}
				}

	...
	struct msm_isp_cmd *cmd)
{
	...
	switch (cmd->id) {
	case VFE_CMD_RESET:
		exisp_err_flag = 0;	/* exisp_code npdc300053106  */
...
static void vfe32_process_error_irq(uint32_t errStatus)
{
	...
	if (errStatus & VFE32_IMASK_CAMIF_ERROR) {
		exisp_err_flag = 1;	/* exisp_code npdc300053106  */
		pr_err("vfe32_irq: camif errors\n");
		reg_value = msm_camera_io_r(
				vfe32_ctrl->vfebase + VFE_CAMIF_STATUS);
		pr_err("camifStatus  = 0x%x\n", reg_value);
//		vfe32_send_isp_msg(vfe32_ctrl, MSG_ID_CAMIF_ERROR);		/* npdc300053106  */
	}
...
static void axi32_do_tasklet(unsigned long data)
{
	...
	while (atomic_read(&irq_cnt)) {
		...
		if (qcmd->vfeInterruptStatus0 &
				VFE_IRQ_STATUS0_CAMIF_SOF_MASK) {
			exisp_sof_notify_chk(&axi_ctrl->subdev);
		}
#endif
...
#if 1	/* EXISP_code debug */
	if(exisp_err_flag == 1){
		CDBG("### msm_axi_process_irq SKIP \n");
		return;
	}
#endif

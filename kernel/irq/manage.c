...
static int synchronize_irq_limited(unsigned int irq, unsigned int count)
{
	struct irq_desc *desc = irq_to_desc(irq);
	bool inprogress;
	...
	do {
		 ...
		 * give the wrong answer due to the lack of memory barriers.
		 */
		while (irqd_irq_inprogress(&desc->irq_data) && ( count ? (--count) : (unsigned int) 0 ))
			cpu_relax();
		...
		/* Oops, that failed? */
	} while (inprogress && ( count ? (--count) : (unsigned int) 0 ));
...
#ifdef CONFIG_FB_MSM_MIPI_DSI_TMD
int disable_irq_limited(unsigned int irq, unsigned int count)
{
	if (!__disable_irq_nosync(irq))
		return synchronize_irq_limited(irq, count);
	else
		return -EINVAL;
}
EXPORT_SYMBOL(disable_irq_limited);
#endif
